...
class DarkPrep:
    ...
    def check_params(self):
        ...
        if self.params["Inst"]["instrument"].lower() not in INST_LIST:
            raise ValueError(
                (
                    "WARNING: {} instrument not implemented within "
        ...
        if self.params["Inst"]["use_JWST_pipeline"] is False:
            if self.params["Reffiles"]["linearized_darkfile"].lower() == "none":
                raise ValueError(
                    (
                        "WARNING: You specified no use of the JWST pipeline, but "
        ...
        # input to work with.
        if (
            self.params["Reffiles"]["linearized_darkfile"].lower() == "none"
            and self.params["Reffiles"]["dark"].lower() == "none"
        ):
            raise ValueError(
                ("WARNING: No raw nor linearized dark file given. Unable to proceed.")
            )
        ...
        self.runStep = {}
        self.runStep["badpixmask"] = self.check_run_step(
    ...
    def crop_dark(self, model):
        ...
        if (
            ...
            or (self.subarray_bounds[2] != (xd - 1))
            or (self.subarray_bounds[1] != 0)
            or (self.subarray_bounds[3] != (yd - 1))
        ):
            ...
            if len(modshape) == 4:
                ...
                try:
                    model.sbAndRefpix = model.sbAndRefpix[
                        :,
                        :,
                        self.subarray_bounds[1] : self.subarray_bounds[3] + 1,
                        self.subarray_bounds[0] : self.subarray_bounds[2] + 1,
                    ...
                    pass
                try:
                    model.zeroframe = model.zeroframe[
                        :,
                        self.subarray_bounds[1] : self.subarray_bounds[3] + 1,
                        self.subarray_bounds[0] : self.subarray_bounds[2] + 1,
                    ]
        ...
        if self.params["Readout"]["namp"] != 4 and self.params["Readout"]["namp"] != 1:
            raise ValueError(
                (
                    "ERROR: amplifier mode specified ({}) is not allowed".format(
        ...
        if self.params["Readout"]["namp"] == 4:
            ...
            if n != nfast:
                raise ValueError(
                    (
                        "ERROR: 4 amplifier mode specified but the number of pixels "
                        "in the fast read direction ({}) is not a multiple of 4.".format(
                            nfast
                        )
                    )
                )
    ...
    def darkints(self):
        ...
        if reqints <= ndarkints:
            ...
            logger.info(
                (
                    "Requested output has {} integrations, while input dark has only {}. "
                    "Adding integrations to input dark by making copies of the input.".format(
                        reqints, ndarkints
                    )
                )
            )
    ...
    def data_volume_check(self, obj):
        ...
        if ngroup * (nskip + nframe) > inputframes:
            self.logger.warning(
                (
                    "Not enough frames in the input integration to "
                    ...
                    "of {} frames.".format(
                        inputframes, ngroup, nframe, nskip, ngroup * (nframe + nskip)
                    )
                )
            )
            self.logger.warning(
                (
                    "Making copies of {} dark current frames and adding them to "
                    "the end of the dark current integration.".format(
                        ngroup * (nskip + nframe) - inputframes
                    )
                )
            )
            ...
            for ints in range(div - 1):
                ...
                if obj.sbAndRefpix is not None:
                    ...
                    obj.sbAndRefpix = np.hstack(
                        (obj.sbAndRefpix, extra_sb + obj.sbAndRefpix[:, -1, :, :])
                    )
            ...
            extra_frames = (
                np.copy(obj.data[:, 1 : mod + 1, :, :]) - obj.data[:, 0, :, :]
            )
            obj.data = np.hstack((obj.data, extra_frames + obj.data[:, -1, :, :]))
            if obj.sbAndRefpix is not None:
                extra_sb = (
                    np.copy(obj.sbAndRefpix[:, 1 : mod + 1, :, :])
                    - obj.sbAndRefpix[:, 0, :, :]
                )
                obj.sbAndRefpix = np.hstack(
                    (obj.sbAndRefpix, extra_sb + obj.sbAndRefpix[:, -1, :, :])
            ...
            if obj.sbAndRefpix is not None:
                obj.sbAndRefpix = obj.sbAndRefpix[
                    :, 0 : ngroup * (nskip + nframe), :, :
                ]
    ...
    def get_base_dark(self, input_file):
        ...
        if not local:
            try:
                self.logger.info(
                    "Local copy of dark current file {} not found. Attempting to download".format(
                        input_file
                    )
                )
                ...
                self.logger.error(
                    "Local copy of dark current file {} not found, and unable to download. Quitting.".format(
                        input_file
                    )
                )
        ...
        darkrange = 1.0 * self.dark.data.max() - 1.0 * self.dark.data.min()
        if darkrange > 65535.0:
            raise ValueError(
                (
                    "WARNING: Range of data values in the input dark is too large. "
        ...
        if self.dark.header["READPATT"] not in ["RAPID", "NISRAPID", "FGSRAPID"]:
            if (
                self.params["Readout"]["readpatt"].upper()
                != self.dark.header["READPATT"]
            ):
                self.logger.error(
                    (
                        "WARNING: cannot transform input {} integration into "
                        "output {} integration.".format(
                            self.dark.header["READPATT"],
                ...
                raise ValueError(
                    (
                        "Only RAPID, NISRAPID, or FGSRAPID inputs can be translated to a "
    ...
    def integration_copy(self, req, darkint):
        ...
        if extras > 0:
            self.dark.data = np.vstack(
                (self.dark.data, self.dark.data[0:extras, :, :, :])
            )
            if self.dark.sbAndRefpix is not None:
                self.dark.sbAndRefpix = np.vstack(
                    (self.dark.sbAndRefpix, self.dark.sbAndRefpix[0:extras, :, :, :])
                )
            if self.dark.zeroframe is not None:
                self.dark.zeroframe = np.vstack(
                    (self.dark.zeroframe, self.dark.zeroframe[0:extras, :, :])
                )
    ...
    def linearize_dark(self, darkobj, save_refs=True):
        ...
        self.logger.info(
            "Creating a linearized version of the dark current input ramp using JWST calibration pipeline."
        )

        # Run the DQ_Init step
        if self.runStep["badpixmask"]:
            linDark = DQInitStep.call(
                dark, override_mask=self.params["Reffiles"]["badpixmask"]
            )
        else:
        ...
        if self.runStep["saturation_lin_limit"]:
            linDark = SaturationStep.call(
                linDark, override_saturation=self.params["Reffiles"]["saturation"]
            )
        else:
        ...
        if self.runStep["superbias"]:
            linDark = SuperBiasStep.call(
                linDark, override_superbias=self.params["Reffiles"]["superbias"]
            )
        else:
        ...
        if self.runStep["linearity"]:
            linDark = LinearityStep.call(
                linDark, override_linearity=self.params["Reffiles"]["linearity"]
            )
        else:
    ...
    def prepare(self):
        ...
        self.basename = os.path.join(
            self.params["Output"]["directory"], base_name[0:-5]
        )
        ...
        if self.file_splitting:
            ...
            split_seed_frames, seg_ind, int_ind = find_file_splits(
                xdim,
                ydim,
            ...
            forced_ints_per_file = int(self.frames_per_integration / self.numgroups) * (
                int_ind[1] - int_ind[0]
            )
            (
                split_seed,
            ...
            self.logger.info(
                "File splitting enabled. Starting integration numbers for all file segments: {}".format(
                    integration_segment_indexes
                )
            )
        else:
            self.logger.info(
                "File-splitting disabled. Output will be forced into a single file."
            )
            split_seed = False
        ...
        if len(integration_segment_indexes) > 2:
            delta_int = (
                integration_segment_indexes[1:] - integration_segment_indexes[0:-1]
            )
            if delta_int[-1] == 1 and delta_int[0] != 1:
                integration_segment_indexes[-2] -= 1
                self.logger.info(
                    "Adjusted final segment file to avoid single integration: {}".format(
                        integration_segment_indexes
                    )
                )
        ...
        if len(integration_segment_indexes[:-1]) > 1:
            self.logger.info(
                (
                    "An estimate of processing time remaining will be provided after the first segment "
                    "has been completed.\n\n"
                )
            )
        ...
        for seg_index, segment in enumerate(integration_segment_indexes[:-1]):
            ...
            if number_of_ints == 1:
                ...
                try:
                    random_indices = np.random.choice(
                        len(dark_list), number_of_ints, replace=False
                    )
            ...
            for file_index, filename in enumerate(files_to_use):
                ...
                self.logger.info(
                    (
                        "DARK has been reordered to {} to match the input readpattern of {}".format(
                            self.dark.data.shape, self.dark.header["READPATT"]
                        )
                    )
                )
                ...
                if (self.params["Inst"]["use_JWST_pipeline"]) & (
                    ...
                    self.logger.info(
                        "Linearized dark shape: {}".format(self.linDark.data.shape)
                    )
                    ...
                    if self.params["Readout"]["readpatt"].upper() in [
                        ...
                        self.logger.info(
                            (
                                "Output is {}, grabbing zero frame from linearized dark".format(
                                    self.params["Readout"]["readpatt"].upper()
                                )
                            )
                        )
                        ...
                        self.logger.info(
                            (
                                "Now we need to linearize the zeroframe because the "
                        ...
                        # pipeline steps
                        self.zeroModel = read_fits.Read_fits()
                        self.zeroModel.data = np.expand_dims(
                            self.dark.zeroframe, axis=1
                        )
                        self.zeroModel.header = self.linDark.header
                        self.zeroModel.header["NGROUPS"] = 1
                        self.zeroModel = self.linearize_dark(
                            self.zeroModel, save_refs=False
                        )
                        ...
                        self.logger.info(
                            (
                                "Non-RAPID input dark with no zeroframe extension."
                        ...
                        self.zeroModel.sbAndRefpix = np.expand_dims(
                            self.linDark.sbAndRefpix[0, 0, :, :], axis=0
                        )
                        for i in range(2, nint + 1):
                            self.zeroModel.data = np.vstack(
                                (self.zeroModel.data, np.expand_dims(zframe, axis=0))
                            )
                            self.zeroModel.sbAndRefpix = np.vstack(
                                (
                                    self.zeroModel.sbAndRefpix,
                                    np.expand_dims(
                                        self.linDark.sbAndRefpix[0, 0, :, :], axis=0
                                    ),
                                )
                ...
                if file_index == 0:
                    ...
                    final_dark = np.zeros(
                        (number_of_ints, num_grps, ydim, xdim), dtype=np.float32
                    )
                    final_sbandrefpix = np.zeros(
                        (number_of_ints, num_grps, ydim, xdim), dtype=np.float32
                    )
                    final_zerodata = np.zeros(
                        (number_of_ints, ydim, xdim), dtype=np.float32
                    )
                    final_zero_sbandrefpix = np.zeros(
                        (number_of_ints, ydim, xdim), dtype=np.float32
                    )

                if not use_all_files:
                    self.logger.info(
                        "Setting integration {} to use file {}\n".format(
                            file_index, os.path.split(filename)[1]
                        )
                    )
                    ...
                    # frames = np.where(mapping == file_index)[0]
                    self.logger.info(
                        "File number {} will be used for integrations {}".format(
                            file_index, frames
                        )
                    )
            ...
            if split_seed:
                objname = self.basename + "_seg{}_linear_dark_prep_object.fits".format(
                    str(seg_index + 1).zfill(3)
                )
            else:
            ...
            self.logger.info(
                "\n\nSegment {} out of {} complete.".format(
                    seg_index + 1, len(integration_segment_indexes[:-1])
                )
            )
            if len(integration_segment_indexes[:-1]) > 1:
                ...
                estimated_remaining_time = (
                    time_per_segment
                    * (len(integration_segment_indexes[:-1]) - (seg_index + 1))
                    * u.second
                )
                ...
                self.logger.info(
                    (
                        "Estimated time remaining in dark_prep: {} minutes. "
    ...
    def read_linear_dark(self, input_file):
        ...
        try:
            self.logger.info(
                ("Reading in linearized dark current ramp from {}".format(input_file))
            )
    ...
    def reorder_dark(self, dark):
        ...
        if (darkpatt in rapids) & (dark.zeroframe is None):
            ...
            self.logger.info(
                "Unable to save the zeroth frame because the input dark current ramp is not RAPID."
            )
            sbzero = None
        ...
        if (darkpatt in rapids) and (self.params["Readout"]["readpatt"] not in rapids):
            ...
            for integ in range(dark.data.shape[0]):
                ...
                for i in range(self.params["Readout"]["ngroup"]):
                    ...
                    self.logger.info(
                        (
                            "Averaging dark current ramp. Frames {}, to become "
            ...
            raise ValueError(
                (
                    "WARNING: dark current readout pattern is {} and requested "
                    "output is {}. Cannot convert between the two.".format(
