...
class GrismTSO:
    def __init__(
        ...
        parameter_file,
        SED_file=None,
        SED_normalizing_catalog_column=None,
        final_SED_file=None,
        save_dispersed_seed=True,
        source_stamps_file=None,
        extrapolate_SED=True,
        override_dark=None,
        disp_seed_filename=None,
        orders=["+1", "+2"],
        lightcurves=None,
        lightcurve_times=None,
        lightcurve_wavelengths=None,
    ):
        """
        Parameters
        ----------
        ...
        if self.datadir is None:
            raise ValueError(
                (
                    "WARNING: {} environment variable is not set."
    ...
    def create(self):
        ...
        (
            ...
            self.frames_per_int,
            self.total_frames,
        ) = utils.get_frame_count_info(
            self.numints, self.numgroups, self.numframes, self.numskips, self.numresets
        )
        ...
        self.logger.info(
            "Splitting background and TSO source into multiple yaml files."
        )
        self.logger.info("Running background sources through catalog_seed_image.")
        self.logger.info(
            "background param file is: {}".format(self.background_paramfile)
        )
        ...
        bkgd_waves, bkgd_fluxes = backgrounds.get_1d_background_spectrum(
        ...
        for seed_file in background_seed_files:
            if seed_file is not None:
                ...
                bkgd_output_file = "{}_background_image.fits".format(
                    orig_parameters["Output"]["file"].split(".fits")[0]
                )
                background_image_filename = os.path.join(
                    orig_parameters["Output"]["directory"], bkgd_output_file
                )
                disp = self.run_disperser(
                    seed_file,
                    orders=self.orders,
                    ...
                    finalize=True,
                    background_image_output=background_image_filename,
                )
        ...
        if self.lightcurves is None:
            ...
            lightcurves, times = self.make_lightcurves(
                tso_catalog, self.frametime, transmission_spectrum
            )
        else:
            if self.lightcurve_times is None:
                raise ValueError(
                    (
                        "User-provided lightcurves are present, but associated times are not (using "
            ...
            if self.lightcurve_wavelengths is None:
                raise ValueError(
                    (
                        "User-provided lightcurves are present, but associated wavelengths are not (using "
            ...
            if len(self.lightcurves.shape) != 2:
                raise ValueError(
                    (
                        "User-provided lightcurves needs to be a 2D numpy array with dimensions."
                    )
                )

            self.logger.info(
                (
                    "User-input 2D array of lightcurves, lightcurve times, and wavelengths "
            ...
            if isinstance(self.lightcurve_times, Quantity):
                ...
                self.logger.info(
                    "No units associated with lightcurve_times. Assuming seconds."
                )
            ...
            if isinstance(self.lightcurve_wavelengths, Quantity):
                ...
                self.logger.info(
                    "No units associated with lightcurve_wavelengths. Assuming microns."
                )
            ...
            lc_function = interp2d(
                lightcurve_wavelengths, lightcurve_times, self.lightcurves
            )
        ...
        self.logger.info(
            "Frame numbers containing the transit: {} - {}".format(
        ...
        grism_seed_object = self.run_disperser(
            tso_direct.seed_file,
            orders=self.orders,
        ...
        no_transit_signal = utils.crop_to_subarray(
        ...
        full_maskimage[4 : tso_direct.ffsize - 4, 4 : tso_direct.ffsize - 4] = 1.0
        refpix_mask = self.crop_to_aperture(
            orig_parameters, tso_direct.subarray_bounds, full_maskimage
        )
        ...
        groups_per_segment = (
            self.grp_segment_indexes[1:] - self.grp_segment_indexes[:-1]
        )
        ...
        for i, int_dim in enumerate(ints_per_segment):
            ...
            for j, grp_dim in enumerate(groups_per_segment):
                ...
                self.logger.info(
                    "\n\nCurrent segment part contains: {} integrations and {} groups.".format(
                        int_dim, grp_dim
                    )
                )
                self.logger.info("Creating frame by frame dispersed signal")
                segment_seed = np.zeros(
                    (int_dim, grp_dim, self.seed_dimensions[0], self.seed_dimensions[1])
                )
                ...
                for integ in np.arange(int_dim):
                    ...
                    for frame in np.arange(grp_dim):
                        ...
                        if total_frame_counter in unaltered_frames:
                            frame_only_signal = (
                                background_dispersed + no_transit_signal
                            ) * self.frametime
                        ...
                        # cached disperser with the appropriate lightcurve
                            ...
                            trans_interp = interp1d(
                                transmission_spectrum["Wavelength"], frame_transmission
                            )

                            for order in self.orders:
                                grism_seed_object.this_one[
                                    order
                                ].disperse_all_from_cache(trans_interp)
                            ...
                            frame_only_signal = (
                                background_dispersed + cropped_grism_seed_object
                            ) * self.frametime
                        ...
                        segment_seed[integ, frame, :, :] = (
                            previous_frame + frame_only_signal
                        )
                ...
                self.logger.info(
                    "Segment int and frame start numbers: {} {}".format(
                        self.segment_int_start_number, self.segment_frame_start_number
                    )
                )
                ...
                if (
                    orig_parameters["Readout"]["array_name"]
                    not in self.fullframe_apertures
                ):
                    self.logger.info(
                        "Dispersed seed image size: {}".format(segment_seed.shape)
                    )
                ...
                self.save_seed(
                    segment_seed, tso_segmentation_map, tso_seed_header, orig_parameters
                )  # ,
    ...
    def file_splitting(self):
        ...
        if self.split_seed:
            forced_ints_per_file = int(self.frames_per_int / self.numgroups) * (
                self.int_segment_indexes[1] - self.int_segment_indexes[0]
            )
            (
                split_seed_g,
                self.group_segment_indexes_g,
            ...
            if len(self.file_segment_indexes) > 2:
                ...
                if delta_int[-1] == 1 and delta_int[0] != 1:
                    ...
                    self.logger.info(
                        "Adjusted to avoid single integration: ",
                        self.file_segment_indexes,
                    )
        ...
        self.total_seed_segments_and_parts = (len(self.int_segment_indexes) - 1) * (
            len(self.grp_segment_indexes) - 1
        )
    ...
    def get_param_info(self):
        ...
        for cattype in CATALOG_YAML_ENTRIES:
            if cattype in supported_bkgd_types:
                ...
                if parameters["simSignals"][cattype].lower() != "none":
                    self.logger.info(
                        parameters["simSignals"][cattype].lower(),
                        type(parameters["simSignals"][cattype]),
                    )
                    raise ValueError(
                        "{} catalog: {} is unsupported in grism TSO mode.".format(
                            cattype, parameters["simSignals"][cattype]
                        )
                    )
        ...
        if len(self.catalog_files) == 0:
            ...
            dummy_ptsrc = catalog_generator.PointSourceCatalog(ra=[0.0], dec=[-89.0])
            dummy_ptsrc.add_magnitude_column(
                [40],
                instrument="nircam",
        ...
        if self.instrument == "niriss":
            ...
            raise ValueError(
                "ERROR: Grism TSO mode not supported for {}".format(self.instrument)
            )
    ...
    def make_lightcurves(self, catalog, frame_time, transmission_spec):
        ...
        params.a = catalog[
            "Semimajor_axis_in_stellar_radii"
        ...
        params.t0 = (
            (catalog["Time_of_inferior_conjunction"][0] * time_units).to(u.second).value
        )  # time of inferior conjunction
        params.per = (
            (catalog["Orbital_period"][0] * time_units).to(u.second).value
        )  # orbital period
    ...
    def make_frame_times(self, catalog):
        ...
        time = np.arange(
            start_time, end_time, self.frametime
    ...
    def param_checks(self):
        ...
        if self.orders not in [["+1"], ["+2"], ["+1", "+2"], None]:
            raise ValueError(
                (
                    "ERROR: Orders to be dispersed must be either None or some combination "
                    "of '+1', '+2'"
                )
            )
    ...
    def crop_to_aperture(params, sub_bounds, array):
        ...
        if ap_suffix not in ["FULL", "CEN"]:
            cropped = array[
                sub_bounds[1] : sub_bounds[3] + 1, sub_bounds[0] : sub_bounds[2] + 1
            ]
        return cropped
    ...
    def run_disperser(
        ...
        loc = os.path.join(
            self.datadir,
            "{}/GRISM_{}/current/".format(self.instrument, self.instrument.upper()),
        )
        if not os.path.isdir(loc):
            raise ValueError(
                (
                    ...
                    "https://mirage-data-simulator.readthedocs.io/en/latest/reference_files.html foe details.".format(
                        loc
                    )
                )
            )
        ...
        if finalize:
            if add_background:
                ...
                disp_seed.finalize(
                    BackLevel=scaling_factor, tofits=background_image_output
                )
            else:
    ...
    def save_seed(self, seed, segmentation_map, seed_header, params):
        ...
        if len(arrayshape) == 4:
            ...
            tgroup = self.frametime * (
                params["Readout"]["nframe"] + params["Readout"]["nskip"]
            )
            self.logger.info("Seed image is 4D.")
        else:
            raise ValueError(
                "Only 4D seed images supported. This seed image is {}D".format(
                    len(arrayshape)
                )
            )
        ...
        if self.total_seed_segments_and_parts == 1:
            self.seed_file = os.path.join(
                self.basename + "_" + params["Readout"][usefilt] + "_seed_image.fits"
            )
        else:
        ...
        seed_header[
            "EXSEGTOT"
        ] = self.total_seed_segments  # Total number of segments in exp
        seed_header["EXSEGNUM"] = self.segment_number  # Segment number of this file
        seed_header[
            "PART_NUM"
        ] = self.segment_part_number  # Segment part number of this file
        ...
        self.logger.info(
            "Seed image and segmentation map saved as {}".format(self.seed_file)
        )
    ...
    def split_param_file(self, params):
        ...
        present = [
            True if params["simSignals"][cat] is not None else False
            for cat in bkgd_cats
        ]
        ...
        if any(present):
            ...
            background_params["Output"]["file"] = params["Output"]["file"].replace(
                ".fits", "_background_sources.fits"
            )
            self.background_paramfile = self.paramfile.replace(
                ".{}".format(suffix), "_background_sources.{}".format(suffix)
            )
        ...
        if params["simSignals"]["tso_grism_catalog"] is None:
            raise ValueError(
                "tso_grism_catalog is not populated in the input yaml. No TSO source to simulate."
            )

        tso_params = copy.deepcopy(params)
        tso_params["Inst"]["mode"] = "wfss"
        tso_params["Output"]["grism_source_image"] = True
        tso_params["simSignals"]["bkgdrate"] = 0.0
        ...
        tso_params["simSignals"]["pointsource"] = tso_params["simSignals"][
            "tso_grism_catalog"
        ]
        tso_params["Output"]["file"] = tso_params["Output"]["file"].replace(
            ".fits", "_tso_grism_sources.fits"
        )

        self.tso_paramfile = self.paramfile.replace(
            ".{}".format(suffix), "_tso_grism_sources.{}".format(suffix)
        )
    ...
    def tso_catalog_check(self, catalog, exp_time):
        ...
        catalog_total_time = (
            catalog["End_time"][0] - catalog["Start_time"][0]
        ) * catalog_time_units
        ...
        if exp_time > catalog_total_time:
            self.logger.info(
                (
                    "WARNING: Lightcurve duration specified in TSO catalog file is less than "
        ...
        if (catalog["Time_of_inferior_conjunction"][0] < catalog["Start_time"][0]) or (
            ...
            raise ValueError(
                (
                    "ERROR: the inferior conjucntion time in the TSO catalog is "
