...
class ReadAPTXML:
    ...
    def __init__(self):
        ...
        self.APTObservationParams_keys = (
            ProposalParams_keys + ObsParams_keys + FilterParams_keys + OtherParams_keys
        )
    ...
    def get_tracking_type(self, observation):
        ...
        if len(matched_key) == 0:
            raise ValueError(
                "No matching target name for {} in self.target_type".format(targname)
            )
        elif len(matched_key) > 1:
            raise ValueError(
                "Multiple matching target names for {} in self.target_type.".format(
                    targname
                )
            )
        else:
    ...
    def read_xml(self, infile, verbose=False):
        ...
        try:
            prop_id = "{:05d}".format(
                int(proposal_info.find(self.apt + "ProposalID").text)
            )
        except:
        ...
        for i_obs, obs in enumerate(observation_list):
            ...
            if template_name not in known_APT_templates:
                ...
                self.logger.info(
                    "No protocol written to read {} template in observation {}. Skipping.".format(
                        template_name, observation_number
                    )
                )
            ...
            if verbose:
                ...
                self.logger.info(
                    "Observation `{}` labelled `{}` uses template `{}`".format(
                        observation_number, label, template_name
                    )
                )
                number_of_entries = len(self.APTObservationParams["Instrument"])
                self.logger.info(
                    "APTObservationParams Dictionary holds {} entries before reading template".format(
                        number_of_entries
                    )
                )
            ...
            if coordparallel == "true":
                try:
                    CoordinatedParallelSet = obs.find(
                        self.apt + "CoordinatedParallelSet"
                    ).text
                except AttributeError:
            ...
            visit_numbers = [
                int(element.items()[0][1])
                for element in obs
                if element.tag.split(self.apt)[1] == "Visit"
            ]
            ...
            if template_name in [
                ...
                exposures_dictionary = self.read_generic_imaging_template(
                    template,
                    template_name,
                    obs,
                    proposal_parameter_dictionary,
                ...
                if coordparallel == "true":
                    parallel_template_name = etree.QName(
                        obs.find(self.apt + "FirstCoordinatedTemplate")[0]
                    ).localname
                    if parallel_template_name in ["MiriImaging"]:
                        pass
                    else:
                        parallel_exposures_dictionary = self.read_parallel_exposures(
                            obs,
                            exposures_dictionary,
                            proposal_parameter_dictionary,
                            verbose=verbose,
                        )
                        exposures_dictionary = append_dictionary(
                            exposures_dictionary,
                ...
                exposures_dictionary = self.read_niriss_external_calibration_template(
                ...
                exposures_dictionary = self.read_miri_prime_imaging(
                    template, template_name, obs, proposal_parameter_dictionary
                )
                if coordparallel == "true":
                    parallel_template_name = etree.QName(
                ...
                exposures_dictionary = self.read_miri_mrs_template(
                    template, template_name, obs, proposal_parameter_dictionary
                )

            elif template_name == "NirspecMOS":
                exposures_dictionary = self.read_nirspec_mos(
                    template, template_name, obs, proposal_parameter_dictionary
                )
                if coordparallel == "true":
                    parallel_template_name = etree.QName(
                ...
                exposures_dictionary = self.read_nirspec_ifu(
                    template, template_name, obs, proposal_parameter_dictionary
                )

            # If template is WFSC Commissioning
            elif template_name in ["WfscCommissioning"]:
                exposures_dictionary, num_WFCgroups = self.read_commissioning_template(
                    template, template_name, obs, prop_params
                )

            # If template is WFSC Global Alignment
            elif template_name in ["WfscGlobalAlignment"]:
                exposures_dictionary, n_exp = self.read_globalalignment_template(
                    template, template_name, obs, prop_params
                )
            ...
            # If template is WFSC Coarse Phasing
                ...
                (
                    exposures_dictionary,
                    n_tiles_phasing,
                ) = self.read_coarsephasing_template(
                    template, template_name, obs, prop_params
                )

            # If template is WFSC Fine Phasing
            elif template_name in ["WfscFinePhasing"]:
                exposures_dictionary, n_tiles_phasing = self.read_finephasing_template(
                    template, template_name, obs, prop_params
                )

            # If template is NIRCam Grism Time Series
            elif template_name == "NircamGrismTimeSeries":
                exposures_dictionary = self.read_nircam_grism_time_series(
                    template, template_name, obs, proposal_parameter_dictionary
                )

            # If template is NIRCam Imaging Time Series
            elif template_name == "NircamTimeSeries":
                exposures_dictionary = self.read_nircam_imaging_time_series(
                    template, template_name, obs, proposal_parameter_dictionary
                )

            # NIRISS AMI
            elif template_name == "NirissAmi":
                exposures_dictionary = self.read_niriss_ami_template(
                    template,
                    template_name,
                    obs,
                    proposal_parameter_dictionary,
            ...
            # If template is WFSS
                ...
                exposures_dictionary = self.read_nircam_wfss_template(
                    template, template_name, obs, proposal_parameter_dictionary
                )
                if coordparallel == "true":
                    parallel_template_name = etree.QName(
                    ...
                    if parallel_template_name in ["MiriImaging"]:
                        ...
                        raise ValueError(
                            "Parallel template {} (with primary template {}) not supported.".format(
                                parallel_template_name, template_name
                            )
                        )

            elif template_name == "NirissWfss":
                exposures_dictionary = self.read_niriss_wfss_template(
                    template,
                    template_name,
                    obs,
                    proposal_parameter_dictionary,
                ...
                exposures_dictionary = self.read_nircam_coronagraphy_template(
                    template, template_name, obs, proposal_parameter_dictionary
                )
                ...
                exposures_dictionary = self.read_miri_coronagraphy_template(
                    template, template_name, obs, proposal_parameter_dictionary
                )

            else:
                self.logger.info(
                    "SKIPPED: Observation `{}` labelled `{}` uses template `{}`".format(
                        observation_number, label, template_name
                    )
                )
                continue
            ...
            if verbose:
                self.logger.info(
                    "Dictionary read from template has {} entries.".format(
                        len(exposures_dictionary["Instrument"])
                    )
                )
            ...
            self.APTObservationParams = append_dictionary(
            ...
            tile_state = np.array(
                [
                    mosaic_tiles[i].find(".//" + self.apt + "TileState").text
                    for i in range(len(mosaic_tiles))
                ]
            )
            n_tiles = np.sum(
                np.array(
                    [
                        True if state == "Tile Included" else False
            ...
            if verbose:
                self.logger.info(
                    "Found {} tile(s) for observation {} {}".format(
                        n_tiles, observation_number, label
                    )
                )
                if len(visit_numbers) > 0:
                    self.logger.info(
                        "Found {} visits with numbers: {}".format(
                            len(visit_numbers), visit_numbers
                        )
                    )
            ...
            if verbose:
                ...
                self.logger.info(
                    "APTObservationParams Dictionary holds {} entries after reading template ({:+d} entries)".format(
                        number_of_entries_after,
                        number_of_entries_after - number_of_entries,
                    )
                )
    ...
    def read_generic_imaging_template(
        self,
        template,
        template_name,
        obs,
        proposal_parameter_dictionary,
        prime_exposure_dict=None,
        verbose=False,
        parallel=False,
    ):
        ...
        if parallel:
            ...
            if verbose:
                self.logger.info(
                    "Prime: {}   Parallel: {}".format(prime_instrument, instrument)
                )
            prime_template = obs.find(self.apt + "Template")[0]
            prime_template_name = etree.QName(prime_template).localname
            prime_ns = "{{{}/Template/{}}}".format(
                self.apt.replace("{", "").replace("}", ""), prime_template_name
            )
            if verbose:
                self.logger.info(
                    "PRIME TEMPLATE NAME IS: {}".format(prime_template_name)
                )
        else:
        ...
        ns = "{{{}/Template/{}}}".format(
            self.apt.replace("{", "").replace("}", ""), template_name
        )
        ...
        if instrument.lower() == "nircam":
            ...
            if primary_dithers_present:
                ...
                if (template_name == "NircamEngineeringImaging") and (
                    number_of_primary_dithers == "2PLUS"
                ):
                ...
                self.logger.info(
                    "Primary dither element {} not found, use default primary dithers value (1).".format(
                        dither_key_name
                    )
                )
            ...
            if not parallel:
                if "-WITH-MIRI" in observation_dict["SubpixelDitherType"]:
                    ...
                    number_of_subpixel_dithers = int(
                        observation_dict["SubpixelDitherType"][0]
                    )
                elif "-WITH-NIRISS" in observation_dict["SubpixelDitherType"]:
                    number_of_subpixel_dithers = int(
                        observation_dict["SubpixelDitherType"][0]
                    )
                elif observation_dict["SubpixelDitherType"] in [
                    "STANDARD",
                    "IMAGING",
                    "SMALL-GRID-DITHER",
                ]:
                    number_of_subpixel_dithers = int(
                        observation_dict["SubpixelPositions"]
                    )
            else:
                ...
                if prime_instrument == "NIRISS" and prime_template_name == "NirissWfss":
                    observation_dict["PrimaryDithers"] = prime_template.find(
                        prime_ns + dither_key_name
                    ).text
                    ...
                    try:
                        ...
                        observation_dict["PrimaryDitherType"] = copy.deepcopy(
                            observation_dict["PrimaryDithers"]
                        )
                        observation_dict["PrimaryDithers"] = observation_dict[
                            "PrimaryDithers"
                        ][0]
                    observation_dict["DitherSize"] = prime_template.find(
                        prime_ns + "DitherSize"
                    ).text
                    number_of_primary_dithers = int(
                        observation_dict["PrimaryDithers"][0]
                    )
                    number_of_subpixel_dithers = 1

            # Combine primary and subpixel dithers
            number_of_dithers = str(
                int(number_of_primary_dithers) * int(number_of_subpixel_dithers)
            )
            if not parallel:
                self.logger.info(
                    "Number of dithers: {} primary * {} subpixel = {}".format(
                        number_of_primary_dithers,
            ...
            try:
                ...
                if dither_direct == "NO_DITHERING":
                    if verbose:
                        self.logger.info(
                            (
                                "NIRISS WFSS parallel and NO_DITHERING set for direct imgages. Adjusting "
                    ...
                    for counter in range(0, len(num_dithers), 3):
                        num_dithers[counter : counter + 3] = [
                            "1",
                            num_dithers[counter + 1],
                            "1",
                        ]
            except:
                pass
            ...
            for element in template:
                ...
                if element_tag_stripped == "DitherPatternType":
                    ...
                    if dithers.lower() == "none":
                        ...
                        if template_name == "MiriMRS":
                            dither_options = self.get_miri_mrs_dither_options(
                                template, ns
                            )
                ...
                if (
                    (
                        ...
                        and (element_tag_stripped == "ExposureList")
                    )
                    | (
                        (instrument.lower() == "fgs")
                        and (element_tag_stripped == "Exposures")
                    )
                    | (
                        (instrument.lower() == "miri")
                        and (element_tag_stripped == "ExposureList")
                    )
                    | (
                    ...
                    for exposure in element.findall(ns + "Exposure"):
                        ...
                        exposure_dict["number_of_dithers"] = exposure_dict[
                            dither_key_name
                        ]
                        ...
                        for exposure_parameter in exposure:
                            ...
                            exposure_dict[
                                parameter_tag_stripped
                            ] = exposure_parameter.text
                        ...
                        for key in self.APTObservationParams_keys:
                            ...
                            if (key in ["PrimaryDithers", "ImageDithers"]) and (
                                str(value) == "None"
                            ):
                                value = "1"
                            ...
                            if key not in self.APTObservationParams_keys:
                                ...
                                if key not in exposures_dictionary.keys():
                                    exposures_dictionary[key] = [
                                        str(exposure_dict[key])
                                    ]
                                else:
                                    exposures_dictionary[key].append(
                                        str(exposure_dict[key])
                                    )
        ...
        for key, item in exposures_dictionary.items():
            if len(item) == 0:
                exposures_dictionary[key] = [0] * len(
        ...
        if (
            prime_template_name.lower() == "nircamwfss"
            and template_name.lower() == "nirissimaging"
        ):
            ...
            for i in range(len(prime_exposure_dict["OutOfField"][0:-1])):
                if not prime_exposure_dict["OutOfField"][i]:
                    outoffield_parallel_mapping.append(
                        prime_exposure_dict["OutOfField"][i]
                    )
                if (
                    prime_exposure_dict["OutOfField"][i]
                    and prime_exposure_dict["OutOfField"][i + 1]
                ):
                    outoffield_parallel_mapping.append(
                        prime_exposure_dict["OutOfField"][i]
                    )
            ...
            exposures_dictionary["OutOfField"] = [False] * len(
                exposures_dictionary["Filter"]
            )

        return exposures_dictionary

    def read_miri_mrs_template(
        self, template, template_name, obs, prop_params, verbose=True
    ):
    ...
    def read_commissioning_template(self, template, template_name, obs, prop_params):
        ...
        for filt in filter_configs:
            ...
            for j in range(num_WFCgroups + 1):
                ...
                exposures_dictionary = self.add_exposure(
                    exposures_dictionary, tup_to_add
                )
            ...
            # Force 4 amp readout
            exposures_dictionary["NumOutputs"] = [amps] * len(
                exposures_dictionary["Instrument"]
            )
    ...
    def read_globalalignment_template(self, template, template_name, obs, prop_params):
        ...
        exposures_dictionary["NumOutputs"] = [4] * len(
            exposures_dictionary["NumOutputs"]
        )

        # Add the target RA and Dec to the exposure dictionary
        exposures_dictionary["TargetRA"] = [self.target_info[target_name][0]] * len(
            exposures_dictionary["NumOutputs"]
        )
        exposures_dictionary["TargetDec"] = [self.target_info[target_name][1]] * len(
            exposures_dictionary["NumOutputs"]
        )
    ...
    def read_finephasing_template(self, template, template_name, obs, prop_params):
        ...
        if sensing_type in ["Fine Phasing", "Both"]:
            ...
            sw_pupils += ["WLM8", "WLP8", "WLP8", "WLM8", "CLEAR", "WLM8", "WLP8"][
                :n_configs_fp
            ]
            sw_filts += ["F212N", "F212N", "WLP4", "WLP4", "WLP4", "F187N", "F187N"][
                :n_configs_fp
            ]
    ...
    def read_miri_prime_imaging(
        ...
        for dither_config_index, dither_config in enumerate(dither_configs):
            ...
            dither_config_dict[
                "Dither {}".format(dither_config_index + 1)
            ] = config_dict
        ...
        for filter_config_index, filter_config in enumerate(filter_configs):
            ...
            # Combine primary and subpixel dithers
            number_of_dithers = str(
                number_of_primary_dithers * number_of_subpixel_dithers
            )
            self.logger.info(
                "Number of dithers: {} primary * {} subpixel = {}".format(
                    number_of_primary_dithers,
    ...
    def read_nircam_grism_time_series(
        self, template, template_name, obs, proposal_parameter_dictionary
    ):
        ...
        exposures_dictionary["PrimaryDithers"] = [
            number_of_primary_dithers,
            number_of_primary_dithers,
        ]
        exposures_dictionary["SubpixelPositions"] = [
            number_of_subpixel_dithers,
            number_of_subpixel_dithers,
        ]
    ...
    def read_nircam_imaging_time_series(
        self, template, template_name, obs, proposal_parameter_dictionary
    ):
    ...
    def read_nircam_wfss_template(
        self, template, template_name, obs, proposal_param_dict
    ):
        ...
        grism_number_of_dithers = str(
            int(primary_dither_grism) * int(subpix_dither_grism)
        )
        ...
        for grism_number, grism in enumerate(grismval):
            ...
            for expseq in expseqs:
                ...
                grism_short_pupil, grism_short_filter = self.separate_pupil_and_filter(
                    grism_short_filter
                )
                ...
                (
                    direct_short_pupil,
                    direct_short_filter,
                ) = self.separate_pupil_and_filter(direct_short_filter)
                direct_long_pupil, direct_long_filter = self.separate_pupil_and_filter(
                    direct_long_filter
                )
                ...
                if direct_done == "true":
                    ...
                    exp_seq_dict["PrimaryDitherType"] = [
                        primary_dither_type_grism,
                        primary_dither_type_direct,
                    ]
                    exp_seq_dict["PrimaryDithers"] = [
                        primary_dither_grism,
                        primary_dither_direct,
                    ]
                    exp_seq_dict["SubpixelPositions"] = [
                        subpix_dither_grism,
                        subpix_dither_direct,
                    ]
                    exp_seq_dict["SubpixelDitherType"] = [
                        subpix_dither_type_grism,
                    ...
                    exp_seq_dict["ObservationID"] = [
                        proposal_param_dict["ObservationID"]
                    ] * 2
                    exp_seq_dict["TileNumber"] = ["1"] * 2
                    exp_seq_dict["APTTemplate"] = [template_name] * 2
                    exp_seq_dict["ObservationName"] = [
                        proposal_param_dict["ObservationName"]
                    ] * 2
                    exp_seq_dict["number_of_dithers"] = [
                        grism_number_of_dithers,
                        direct_number_of_dithers,
                    ]
                ...
                exposures_dictionary = self.append_to_exposures_dictionary(
                    exposures_dictionary, exp_seq_dict, proposal_param_dict
                )
            ...
            exposures_dictionary = self.append_to_exposures_dictionary(
                exposures_dictionary, out_of_field_dict, proposal_param_dict
            )
    ...
    def read_nirspec_mos(
        ...
        for exposure in config_pointing:
            ...
            exposure_dict["ObservationID"] = proposal_parameter_dictionary[
                "ObservationID"
            ]
            exposure_dict["ObservationName"] = proposal_parameter_dictionary[
                "ObservationName"
            ]
            exposure_dict["Instrument"] = instrument
            exposure_dict["TargetID"] = obs.find(self.apt + "TargetID").text.split(" ")[
                1
            ]
    ...
    def read_nirspec_ifu(
        ...
        if ta_dithers > 0:
            exposures_dictionary["ProposalID"].extend(
                [proposal_parameter_dictionary["ProposalID"]] * num_ta
            )
            exposures_dictionary["ObservationID"].extend(
                [proposal_parameter_dictionary["ObservationID"]] * num_ta
            )
            exposures_dictionary["ObservationName"].extend(
                [proposal_parameter_dictionary["ObservationName"]] * num_ta
            )
            exposures_dictionary["Instrument"].extend([instrument] * num_ta)
            exposures_dictionary["TargetID"].extend(
                [obs.find(self.apt + "TargetID").text.split(" ")[1]] * num_ta
            )
            ...
            exposures_dictionary["SubpixelPositions"].extend(
                [number_of_subpixel_dithers] * num_ta
            )
        ...
        # Add exposures
        exposures_dictionary["ProposalID"].extend(
            [proposal_parameter_dictionary["ProposalID"]] * num_exposures
        )
        exposures_dictionary["ObservationID"].extend(
            [proposal_parameter_dictionary["ObservationID"]] * num_exposures
        )
        exposures_dictionary["ObservationName"].extend(
            [proposal_parameter_dictionary["ObservationName"]] * num_exposures
        )
        exposures_dictionary["Instrument"].extend([instrument] * num_exposures)
        exposures_dictionary["TargetID"].extend(
            [obs.find(self.apt + "TargetID").text.split(" ")[1]] * num_exposures
        )
        ...
        exposures_dictionary["PrimaryDithers"].extend(
            [number_of_primary_dithers] * num_exposures
        )
        exposures_dictionary["SubpixelPositions"].extend(
            [number_of_subpixel_dithers] * num_exposures
        )
        exposures_dictionary["ImageDithers"].extend([number_of_dithers] * num_exposures)
        exposures_dictionary["number_of_dithers"].extend(
            [number_of_dithers] * num_exposures
        )
        exposures_dictionary["SubpixelDitherType"].extend(["None"] * num_exposures)
        exposures_dictionary["FiducialPointOverride"].extend(
            [str(False)] * num_exposures
        )
    ...
    def read_nircam_coronagraphy_template(
        self,
        template,
        template_name,
        obs,
        ...
        coron_sci_detector = (
            "A2"
            if coronmask == "MASK210R"
            else "A4"
            if coronmask == "MASKSWB"
            else "A5"
        )
        ...
        if ta_targ.upper() != "NONE":
            ...
            ta_subarray = (
                "SUB" + ("NDA" if ta_brightness == "BRIGHT" else "FSA") + coronmask[4:]
            )
            ...
            self.logger.info(
                f"Read TA exposure parameters {ta_readout}, {ta_groups}; inferred subarray= {ta_subarray}"
            )

        # Do not look for the text attribute yet since this keyword may not be present
        astrometric_confirmation_imaging = template.find(
            ncc + "OptionalConfirmationImage"
        ).text

        if astrometric_confirmation_imaging.upper() == "TRUE":
            number_of_astrometric_dithers = (
                2  # at the initial position for TA, and after move to the occulter
            )
            astrom_readout_pattern = template.find(
                ncc + "ConfirmationReadoutPattern"
            ).text
        ...
        if astrometric_confirmation_imaging.upper() == "TRUE":
            ...
            astrometric_exp_dict["number_of_dithers"] = astrometric_exp_dict[
                dither_key_name
            ]
        ...
        for element in template.find(ncc + "Filters"):
            ...
            if element_tag_stripped == "FilterConfig":
                ...
                exposure_dict["ReadoutPattern"] = element.find(
                    ncc + "ReadoutPattern"
                ).text
        ...
        if astrometric_confirmation_imaging.upper() == "TRUE":
            for key in science_exposures:
                exposures_dictionary[key] = list(exposures_dictionary[key]) + list(
                    astrometric_exposures[key]
                )
            print(
                f"Number of astrometric exposure specs: {len(astrometric_exposures[key])}"
            )

        for key in science_exposures:
            exposures_dictionary[key] = list(exposures_dictionary[key]) + list(
                science_exposures[key]
            )
        print(f"Number of science exposure specs: {len(science_exposures[key])}")

        self.logger.info(
            "Number of dithers for NIRCam coron exposure: {} primary * {} subpixel = {}".format(
                number_of_primary_dithers, number_of_subpixel_dithers, number_of_dithers
            )
        )
        if astrometric_confirmation_imaging.upper() == "TRUE":
            self.logger.info(
                "Number of dithers for astrometric confirmation image: {}".format(
                    number_of_astrometric_dithers
                )
            )
        ...
        self.logger.info(
            f"Total number of exposures for this observation: {len(exposures_dictionary[key])}"
        )
        return exposures_dictionary

    def read_miri_coronagraphy_template(
        self,
        template,
        template_name,
        obs,
        ...
        self.logger.info(
            "Number of dithers for MIRI coron exposure: {} primary * {} subpixel = {}".format(
                number_of_primary_dithers, number_of_subpixel_dithers, number_of_dithers
            )
        )
    ...
    def read_parallel_exposures(
        ...
        if template_name in [
            ...
            parallel_exposures_dictionary = self.read_generic_imaging_template(
                template,
                template_name,
                obs,
                proposal_parameter_dictionary,
            ...
            parallel_exposures_dictionary = self.read_niriss_wfss_template(
                template,
                template_name,
                obs,
                proposal_parameter_dictionary,
            ...
            raise ValueError(
                "Parallel observation template {} not supported.".format(template_name)
            )
        ...
        if (
            parallel_length != exposures_dictionary_length
            and prime_template != "NirspecMOS"
        ):
            raise RuntimeError("Mismatch in the number of parallel observations.")

        return parallel_exposures_dictionary

    def append_to_exposures_dictionary(
        self, exp_dictionary, exposure_seq_dict, prop_param_dict
    ):
        ...
        for key in exposure_seq_dict.keys():
            if key not in self.APTObservationParams_keys:
                ...
                if key not in exp_dictionary.keys():
                    self.logger.info(
                        "Key {} not present in APTObservationParams nor exposures_dictionary".format(
                            key
                        )
                    )
                    ...
                    self.logger.info(
                        "Key {} not present in APTObservationParams".format(key)
                    )
                    if isinstance(exposure_seq_dict[key], list):
            ...
            if element_tag_stripped == "ExposureList":
                ...
                for exposure in element.findall(ns + "Exposure"):
                    ...
                    if direct_imaging.upper() == "TRUE":
                        ...
                        direct_dict["ReadoutPattern"] = exposure_dict[
                            "DirectReadoutPattern"
                        ]
                        direct_dict["Groups"] = exposure_dict["DirectGroups"]
                        direct_dict["Integrations"] = exposure_dict[
                            "DirectIntegrations"
                        ]
                ...
                if ta_targ.upper() != "NONE":
                    for key in science_exposures:
                        exposures_dictionary[key] = list(ta_exposures[key]) + list(
                            science_exposures[key]
                        )
                else:
                    for key in science_exposures:
                ...
                if direct_imaging.upper() == "TRUE":
                    for key in science_exposures:
                        exposures_dictionary[key] = list(
                            exposures_dictionary[key]
                        ) + list(direct_exposures[key])

        self.logger.info(
            "Number of dithers for AMI exposure: {} primary * {} subpixel = {}".format(
                number_of_primary_dithers, number_of_subpixel_dithers, number_of_dithers
            )
        )
        if direct_imaging.upper() == "TRUE":
            self.logger.info(
                "Number of dithers for direct image: {}".format(
                    number_of_direct_dithers
                )
            )
    ...
    def read_niriss_external_calibration_template(
        self, template, template_name, obs, proposal_parameter_dictionary, verbose=False
    ):
        """Read imaging template content regardless of instrument.
        ...
        self.logger.info(
            "Number of dithers for NIRISS External Calibration exposure: {} primary * {} subpixel = {}".format(
                primary_dithers, subpixel_positions, number_of_dithers
            )
        )
        for key in exposures_dictionary.keys():
            if type(exposures_dictionary[key]) is not list:
    ...
    def read_niriss_wfss_template(
        ...
        if parallel:
            ...
            if prime_instrument.upper() == "NIRCAM":
                ...
                pdither_type_grism = prime_template.find(
                    prime_ns + "PrimaryDitherType"
                ).text
                dither_direct = prime_template.find(
                    prime_ns + "DitherNirissWfssDirectImages"
                ).text
                sdither_type_grism = prime_template.find(
                    prime_ns + "CoordinatedParallelSubpixelPositions"
                ).text
                try:
                    sdither_grism = str(int(sdither_type_grism[0]))
                except ValueError:
                    sdither_grism = prime_template.find(
                        prime_ns + "SubpixelPositions"
                    ).text
        ...
        for expseq in expseqs:
            ...
            for grism_number, grism in enumerate(grismval):
                ...
                exp_seq_dict["Subarray"] = [
                    "FULL"
                ] * repeats  # Niriss WFSS is always full frame
                ...
                exp_seq_dict["FiducialPointOverride"] = [
                    FiducialPointOverride
                ] * repeats
        ...
        if parallel and prime_instrument.upper() == "MIRI":
            exposures_dictionary["PrimaryDitherType"] = prime_exposure_dict[
                "PrimaryDitherType"
            ]
            exposures_dictionary["SubpixelDitherType"] = prime_exposure_dict[
                "SubpixelDitherType"
            ]
            exposures_dictionary["number_of_dithers"] = [0] * len(
                exposures_dictionary["Instrument"]
            )
            exposures_dictionary["PrimaryDithers"] = [0] * len(
                exposures_dictionary["Instrument"]
            )
            exposures_dictionary["SubpixelPositions"] = [1] * len(
                exposures_dictionary["Instrument"]
            )
...
def get_guider_number(xml_file, observation_number):
    ...
    raise RuntimeError(
        "Could not find guider number in observation {} in {}".format(
            observation_number, xml_file
        )
    )
...
def get_guider_number_from_special_requirements(apt_namespace, obs):
    ...
    sr = [
        x
        for x in obs.iterchildren()
        if x.tag.split(apt_namespace)[1] == "SpecialRequirements"
    ][0]
    try:
        gs = [
            x
            for x in sr.iterchildren()
            if x.tag.split(apt_namespace)[1] == "GuideStarID"
        ][0]
    except IndexError:
        raise IndexError(
            "There is no Guide Star Special Requirement for this observation"
        )

    # Pull out the guide star ID and the guider number
    guider = [
        x for x in gs.iterchildren() if x.tag.split(apt_namespace)[1] == "Guider"
    ][0].text
