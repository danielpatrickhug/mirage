...
from mirage.utils.constants import (
    NIRCAM_UNSUPPORTED_PUPIL_VALUES,
    LOG_CONFIG_FILENAME,
...
class AptInput:
    ...
    def __init__(
        ...
        self.config_path = os.path.join(
            pkg_resources.resource_filename("mirage", ""), "config"
        )
    ...
    def add_epochs(self, intab):
        ...
        if self.epoch_list is not None:
            ...
            epochs["pav3"] = [0.0] * len(intab["obs_label"])
    ...
    def add_observation_info(self, intab):
        ...
        OBSERVATION_LIST_FIELDS = (
            "Date PAV3 Filter PointSourceCatalog GalaxyCatalog "
        ...
        unique_instrument_names = [
            name.lower() for name in np.unique(intab["Instrument"])
        ]
            ...
            if instrument == "nircam":
                ...
                for channel in ["SW", "LW"]:
                    for name, item in nircam_mapping.items():
                        ...
                        if (
                            item
                            in "ExtendedScale ExtendedCenter MovingTargetConvolveExtended BackgroundRate".split()
                        ):
                            intab[key].append(entry["FilterConfig"][channel][item])
                        else:
                            intab[key].append(
                                self.full_path(entry["FilterConfig"][channel][item])
                            )
                ...
                for key in OBSERVATION_LIST_FIELDS:
                    ...
                    catalog_names = (
                        "PointSourceCatalog GalaxyCatalog "
    ...
    def create_input_table(self, skip_observations=None, verbose=False):
        ...
        pointing_dictionary = self.get_pointing_info(
            self.pointing_file,
            propid=self.apt_xml_dict["ProposalID"][0],
            skipped_obs_from_xml=self.skip_observations,
        )

        # Check that the .xml and .pointing files agree
        assert len(self.apt_xml_dict["ProposalID"]) == len(
            pointing_dictionary["obs_num"]
        ), (
            "Inconsistent table size from XML file ({}) and pointing file ({}). Something was not "
            "processed correctly in apt_inputs.".format(
                len(self.apt_xml_dict["ProposalID"]),
                len(pointing_dictionary["obs_num"]),
            )
        )

        # Combine the dictionaries
        observation_dictionary = self.combine_dicts(
            self.apt_xml_dict, pointing_dictionary
        )
        ...
        if verbose:
            ...
            for key in observation_dictionary.keys():
                self.logger.info(
                    "{:<25}: number of elements is {:>5}".format(
                        key, len(observation_dictionary[key])
                    )
                )
        ...
        if "WfscGlobalAlignment" in observation_dictionary["APTTemplate"]:
            observation_dictionary = self.global_alignment_pointing(
        ...
        with warnings.catch_warnings():
            ...
            self.exposure_tab = make_start_times(
                self.exposure_tab, offline=self.offline
            )
        ...
        temp_table = Table(
            [
                self.exposure_tab["sequence_id"],
                self.exposure_tab["exposure"],
                self.exposure_tab["act_id"],
                self.exposure_tab["obs_num"],
                self.exposure_tab["detector"],
            ],
            names=("sequence_id", "exposure", "act_id", "obs_num", "detector"),
        )
        ...
        for obs_num in np.unique(self.exposure_tab["obs_num"]):
            for act_id in np.unique(temp_table["act_id"]):
                ...
                for detector in np.unique(temp_table["detector"]):
                    prime_index = np.where(
                        (temp_table["sequence_id"] == "1")
                        & (temp_table["act_id"] == act_id)
                        & (temp_table["obs_num"] == obs_num)
                        & (temp_table["detector"] == detector)
                    )[0]
                    parallel_index = np.where(
                        (temp_table["sequence_id"] == "2")
                        & (temp_table["act_id"] == act_id)
                        & (temp_table["obs_num"] == obs_num)
                        & (temp_table["detector"] == detector)
                    )[0]

                    temp_table["exposure"][prime_index] = [
                        "{:05d}".format(n + 1) for n in np.arange(len(prime_index))
                    ]
                    temp_table["exposure"][parallel_index] = [
                        "{:05d}".format(n + 1) for n in np.arange(len(parallel_index))
                    ]
        ...
        if verbose:
            for key in self.exposure_tab.keys():
                self.logger.info(
                    "{:>20} has {:>10} items".format(key, len(self.exposure_tab[key]))
                )
        ...
        if self.output_csv is None:
            ...
            self.output_csv = os.path.join(
                self.output_dir,
                "Observation_table_for_" + infile.split(".")[0] + ".csv",
            )
        ascii.write(
            Table(self.exposure_tab), self.output_csv, format="csv", overwrite=True
        )
                ...
                if not inst_match_ap:
                    ...
                    if instrument.lower() == "fgs" and aperture[:3] == "NRC":
                        ...
                        if self.exposure_tab["APTTemplate"][i] == "WfscGlobalAlignment":
                            ...
                            self.exposure_tab["APTTemplate"][i]
                            == "FgsExternalCalibration"
                        ):
                            guider_number = read_apt_xml.get_guider_number(
                                self.input_xml, obs_num
                            )
                        else:
                            raise ValueError(
                                "WARNING: unsupported APT template with Fiducial Override."
                            )
                        ...
                        self.logger.error(
                            "{} {} {} {}".format(
                                ...
                                aperture,
                                inst_match_ap,
                                aperture_key[instrument.lower()],
                            )
                        )
                        raise ValueError(
                            "Unknown FiducialPointOverride in program. Instrument = {} but aperture = {}.".format(
                                instrument, aperture
                            )
                        )
                else:
    ...
    def expand_for_detectors(self, input_dictionary):
        ...
        nircam_subarray_file = os.path.join(
            self.config_path, "NIRCam_subarray_definitions.list"
        )
        ...
        for index, instrument in enumerate(input_dictionary["Instrument"]):
            ...
            if instrument == "nircam" and input_dictionary["Mode"][index] != "coron":
                ...
                if len(matches) == 0:
                    raise ValueError(
                        "ERROR: aperture {} in not present in the subarray definition file {}".format(
                            sub, nircam_subarray_file
                        )
                    )
                ...
                for mod in module:
                    good = [
                        ap
                        for ap in matched_allmod_apertures
                        if "NRC{}".format(mod) in ap
                    ]
                ...
                if sub in [
                    ...
                    if sub == "FULL":
                        if mode in ["imaging", "ts_imaging", "wfss"]:
                            ...
                            matched_apertures = np.array(
                                [ap for ap in matched_apertures if "GRISM" in ap]
                            )
                            filtered_aperture = input_dictionary["aperture"][index]
                            filtered_splits = filtered_aperture.split("_")
                            filtered_ap_no_det = "{}_{}".format(
                                filtered_splits[1], filtered_splits[2]
                            )
                        ...
                        matched_apertures = [
                            ap for ap in matched_apertures if sub in ap
                        ]
                    ...
                    filter_dependent_apertures = [
                        ap for ap in matched_apertures if len(ap.split("_")) == 3
                    ]
                ...
                full_apertures = [
                    "NRC{}_{}".format(det, sub)
                    for det, sub in zip(detectors, matched_apertures)
                ]
                ...
                for key in input_dictionary:
                    ...
                    if key not in ["aperture", "detector", "Subarray"]:
                        observation_dictionary[key].extend(
                            ([input_dictionary[key][index]] * num_entries)
                        )
                ...
                if instrument == "niriss":
                    ...
                    if input_dictionary["APTTemplate"][index] == "WfscGlobalAlignment":
                        ...
                        input_dictionary["APTTemplate"][index]
                        == "FgsExternalCalibration"
                    ):
                        guider_number = read_apt_xml.get_guider_number(
                            self.input_xml, input_dictionary["obs_num"][index]
                        )
                    ...
                    instrument == "nircam"
                    and input_dictionary["Mode"][index] == "coron"
                ):
                ...
                for key in input_dictionary:
                    observation_dictionary[key].extend(
                        ([input_dictionary[key][index]] * n_detectors)
                    )
                observation_dictionary["detector"].extend(detectors)
    ...
    def extract_grism_aperture(self, apertures, filter_name):
        ...
        if any(filter_match):
            ...
            self.logger.debug(
                "{} {} {} {}".format(
                    filter_int, aperture_int, min_diff_index, apertures
                )
            )
    ...
    def get_pointing_info(
        self, file, propid=0, skipped_obs_from_xml=None, verbose=False
    ):
                        ...
                        if apt_prd_version != JWST_PRD_VERSION:
                            self.logger.warning(
                                (
                                    ...
                                    "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n".format(
                                        apt_prd_version, JWST_PRD_VERSION
                                    )
                                )
                            )
                    else:
                ...
                # Extract proposal ID
                    ...
                    if line[0:2] == "**":
                        ...
                        if (skip is True) and (verbose):
                            self.logger.info(
                                "Skipping observation {} ({})".format(obsnum, obslabel)
                            )
                    ...
                    try:
                        ...
                        if (
                            ...
                            ("TA" in elements[4])
                            & ("NRC" in elements[4] or "NIS" in elements[4])
                        ):
                            ...
                            observation_id.append(
                                "V{}P{}{}{}{}".format(vid, "00000000", vgrp, seq, act)
                            )
                            # act_counter += 1
        ...
        pointing = {
            ...
            "idlx": idlx,
            "idly": idly,
            "obs_label": observation_label,
            "obs_num": observation_number,
            "visit_num": visit_number,
            "act_id": activity_id,
            "visit_id": visit_id,
            "visit_group": visit_grp,
            "sequence_id": seq_id,
    ...
    def add_options(self, parser=None, usage=None):
        ...
        parser.add_argument(
            "--output_csv",
            help="Name of output file containing list of observations.",
            default=None,
        )
        parser.add_argument(
...
def get_filters(pointing_info):
    ...
    for inst in instrument_list:
        ...
        if inst.upper() == "NIRCAM":
            ...
            for s_filt, s_pup, l_filt, l_pup in zip(
                short_filters, short_pupils, long_filters, long_pupils
            ):
            ...
            short_filter_only = np.where(
                ((short_pupils == "CLEAR") | (short_pupils == "CLEARP"))
            )[0]
...
def make_start_times(obs_info, offline=False):
    ...
    for i, instrument in enumerate(obs_info["Instrument"]):
        ...
        if instrument.lower() in ["miri", "nirspec"]:
            ...
            readpatt_def = config_information["global_readout_patterns"][
                instrument.lower()
            ]
            subarray_def = config_information["global_subarray_definitions"][
                instrument.lower()
            ]
            ...
            if np.sum(match2) == 0:
                raise RuntimeError(
                    (
                        "WARNING!! Readout pattern {} not found in definition file.".format(
                            readpatt
                        )
                    )
                )
            ...
            if np.sum(match) == 0:
                logger.info(
                    (
                        "Aperture: {} does not match any entries in the subarray definition file. Guessing at the "
                        "aperture for the purpose of calculating the exposure time and number of amps.".format(
                            aperture
                        )
                    )
                )
                ...
                aperture = [
                    apername
                    for apername, name in np.array(subarray_def["AperName", "Name"])
                    if (sub in apername) or (sub in name)
                ]
                ...
                if len(aperture) > 1 or len(aperture) == 0 or np.sum(match) == 0:
                    raise ValueError(
                        "Cannot combine detector {} and subarray {}\
                                     into valid aperture name.".format(
                            det, sub
                        )
                    )
            ...
            if amp == 0:
                ...
                logger.info(
                    (
                        "Aperture {} can be used with 1 or 4 readout amplifiers. Defaulting to use 4."
                        "In the future this information should be made a user input.".format(
                            aperture
                        )
                    )
                )
            ...
            if (next_obsid != obsid) | (next_visit != visit):
                ...
                overhead = 0.0  # Reset frame captured in exptime below
            ...
            frametime = utils.calc_frame_time(
                siaf_inst, aperture, siaf_obj.XSciSize, siaf_obj.YSciSize, amp
            )
...
def ra_dec_update(exposure_dict, siaf_instances, verbose=False):
    ...
    for i in range(len(exposure_dict["Module"])):
        ...
        if aperture_name in NIRCAM_LW_GRISMTS_APERTURES:
            ...
            lw_intermediate_aperture = utils.get_lw_grism_tso_intermeidate_aperture(
                aperture_name
            )
        ...
        if "NRCA5_GRISM" in aperture_name and "WFSS" not in aperture_name:
            ...
            (
                ...
                siaf_instances[siaf_instrument],
                aperture_name,
                pointing_ra,
                pointing_dec,
                telescope_roll,
                v2_arcsec=pointing_v2,
                v3_arcsec=pointing_v3,
            )
            ...
            ra, dec = rotations.pointing(
                attitude_matrix, aperture.V2Ref, aperture.V3Ref
            )
