...
config_files = {
    "nircam": {"flux_cal": "NIRCam_zeropoints.list"},
...
class ImgSeed:
    def __init__(
        ...
        cropped_file="cropped_image.fits",
        outdir=None,
        blotted_file=None,
        psf_file=None,
        mosaic_fwhm=None,
        ...
        self.crop_center_ra = 0.0
        self.crop_center_dec = 0.0
        self.channel = None
        self.detector = None
        self.blot_center_ra = 0.0
        self.blot_center_dec = 0.0
        self.blot_pav3 = 0.0
        self.aperture = ""
        ...
        self.outdir = outdir
        self.grism_direct_factor = np.sqrt(2.0)
        ...
        if mosaic_fwhm_units not in allowed_mosaic_fwhm_units:
            raise ValueError(
                (
                    "ERROR: mosaic_fwhm_units must be one of: {}".format(
                        allowed_mosaic_fwhm_units
                    )
                )
            )
        ...
        self.coords = {"x": 1.0, "xoffset": 0.0, "y": 1.0, "yoffset": 0.0}
    ...
    def crop_and_blot(self):
        ...
        if mosaic_fwhm_arcsec > jwst_y_fwhm_arcsec:
            raise ValueError(
                (
                    ...
                    "using photutils. \nMosaic FWHM: {}\nJWST FWHM: {}".format(
                        mosaic_fwhm_arcsec, jwst_y_fwhm_arcsec
                    )
                )
            )
        ...
        # forever
        half_width = 50
        self.jwst_psf = self.jwst_psf[
            mid_y - half_width : mid_y + half_width + 1,
            mid_x - half_width : mid_x + half_width + 1,
        ]
    ...
    def distortion_file_value(self):
        ...
        if self.distortion_file == "crds":
            if self.paramfile is None:
                ...
                if "5" in self.detector:
                    self.reference_dictionary["DETECTOR"] = self.detector.replace(
                        "5", "LONG"
                    )
            ...
            mapping = crds_tools.get_reffiles(
                self.reference_dictionary, ["distortion"], download=True
            )
    ...
    def grism_coord_adjust(self):
        ...
        self.coords["xoffset"] = np.int(
            (self.grism_direct_factor - 1.0)
            * (self.subarr_bounds["xend"] - self.subarr_bounds["xstart"] + 1)
            / 2.0
        )
        self.coords["yoffset"] = np.int(
            (self.grism_direct_factor - 1.0)
            * (self.subarr_bounds["yend"] - self.subarr_bounds["ystart"] + 1)
            / 2.0
        )
    ...
    def make_segmap(self, model):
        ...
        seg_map = detect_sources(model.data, noise * 5.0, 8)
        if seg_map is None:
    ...
    def prepare_psf(self):
        ...
        if self.psf_file is None:
            ...
            if self.gaussian_psf:
                ...
                self.logger.info(
                    "Mosaic pixel scale: {} x {}".format(
                        self.mosaic_metadata["pix_scale1"],
                        self.mosaic_metadata["pix_scale2"],
                    )
                )
                self.logger.info(
                    "JWST pixel scale: {} x {}".format(self.outscale1, self.outscale2)
                )
                self.logger.info(
                    "scale ratios: {}, {}".format(scale_ratio1, scale_ratio2)
                )
                self.logger.info("JWST PSF dims: {} x {}".format(jwst_xdim, jwst_ydim))
                self.logger.info(
                    "Gaussian PSF dimensions: {} x {}".format(gauss_xdim, gauss_ydim)
                )

                # Create 2D Gaussian
                self.mosaic_psf = tools.gaussian_psf(
                    self.mosaic_fwhm, gauss_xdim, gauss_ydim
                )
                ...
                self.mosaic_metadata["telescope"] in KNOWN_PSF_TELESCOPES
                and not self.gaussian_psf
            ):
    ...
    def psf_convolution(self, model):
        ...
        if not np.isclose(
            self.outscale1, self.mosaic_metadata["pix_scale1"], atol=0.0, rtol=0.01
        ):
            orig_jwst = copy.deepcopy(self.jwst_psf)
            self.jwst_psf = resize_psf(
                self.jwst_psf,
                self.outscale1,
                self.mosaic_metadata["pix_scale1"],
                order=3,
        ...
        if (jwst_shape[0] % 2 != mosaic_shape[0] % 2) or (
            jwst_shape[1] % 2 != mosaic_shape[1] % 2
        ):
            raise ValueError(
                (
                    ...
                    "rows/columns to the mosaic PSF. Mosaic PSF shape: {}, JWST PSF shape: {}".format(
                        mosaic_shape, jwst_shape
                    )
                )
            )
        ...
        if self.save_intermediates:
            ...
            hlist = fits.HDUList([h0])
            outfile = os.path.join(
                self.outdir, "{}_convolved_mosaic.fits".format(self.output_base)
            )
            hlist.writeto(outfile, overwrite=True)
    ...
    def add_options(self, parser=None, usage=None):
        ...
        parser.add_argument(
            "paramfile",
            help="File describing the input parameters and instrument settings to use. (YAML format).",
        )
        parser.add_argument(
            "mosaic_file",
            help="Fits file containing the image to create the seed image from",
        )
        parser.add_argument(
            "data_extension_number",
            help="Extension in fits file that contains mosaic image",
            default=0,
        )
        parser.add_argument(
            "wcs_extension_number",
            help="Extension in mosaic fits file that contains WCS information",
            default=0,
        )
        parser.add_argument(
            "cropped_file",
            help="Filename used to save image cropped from mosaic",
            default=None,
        )
        parser.add_argument(
            "blotted_file", help="Filename used to save resampled image", default=None
        )
        parser.add_argument(
            "outdir",
            help="Output directory. If None, the output dir from paramfile is used.",
            default=None,
        )
        parser.add_argument(
            "psf_file",
            help="Name of fits file containing PSF corresponding to mosaic image",
            default=None,
        )
        parser.add_argument(
            "mosaic_fwhm",
            help="FWHM of PSF in mosaic image, in pixels or arcsec",
            default=None,
        )
        parser.add_argument(
            "mosaic_fwhm_units",
            help="Units of PSF FWHM in mosaic image. 'pixels' or 'arcsec'",
            default="arcsec",
        )
        parser.add_argument(
            "gaussian_psf",
            help="If True, create mosaic PSF from Gaussian2D model",
            default=False,
            action="store_true",
        )
        parser.add_argument(
            "save_intermediates",
            help="If True, save intermediate outputs",
            default=False,
            action="store_true",
        )
        return parser
