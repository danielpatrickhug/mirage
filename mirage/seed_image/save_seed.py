...
def save(
    ...
    if len(arrayshape) == 2:
        ...
        g, yd, xd = arrayshape
        tgroup = frametime * (
            parameters["Readout"]["nframe"] + parameters["Readout"]["nskip"]
        )
    ...
    if filename is None:
        ...
        filename = "{}_{}_seed_image.fits".format(
            basename, parameters["Readout"][usefilt]
        )
    ...
    kw["PHOTFNU"] = photfnu
    kw["PHOTPLAM"] = pivot_wavelength * 1.0e4  # put into angstroms
    ...
    if parameters["Inst"]["mode"] in ["wfss", "ts_wfss"]:
        ...
        kw["NOMXSTRT"] = int(fullframe_size * (grism_direct_factor - 1) / 2.0)
        kw["NOMXEND"] = kw["NOMXSTRT"] + fullframe_size - 1
        kw["NOMYSTRT"] = int(fullframe_size * (grism_direct_factor - 1) / 2.0)
        kw["NOMYEND"] = kw["NOMYSTRT"] + fullframe_size - 1

    kw["GRISMPAD"] = grism_direct_factor
    seedinfo = kw
    save_single_fits(
        seed_image, filename, key_dict=kw, image2=segmentation_map, image2type="SEGMAP"
    )
