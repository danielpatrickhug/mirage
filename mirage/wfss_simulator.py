...
class WFSSSim:
            ...
            if seed_files[0] is not None:
                ...
                if not background_done:
                    ...
                    dispersed_objtype_seed.finalize(
                        BackLevel=scaling_factor, tofits=self.background_image_filename
                    )
    ...
    def param_checks(self):
        ...
        if (len(self.paramfiles) > 1) and (self.SED_file is not None):
            raise ValueError(
                (
                    "WARNING: When using an SED file, you must provide only one parameter file."
                )
            )
            ...
            for darkfile in dark_list:
                ...
                if not avail:
                    raise FileNotFoundError(
                        ("WARNING: {} does not exist.".format(darkfile))
                    )
    ...
    def find_param_info(self):
        ...
        for i, pfile in enumerate(self.paramfiles):
            ...
            cats = [
                params["simSignals"][cattype]
                for cattype in CATALOG_YAML_ENTRIES
                if "tso_" not in cattype
            ]
            ...
            if params["Inst"]["mode"].lower() == "wfss":
                ...
                if wfss_files_found == 2:
                    raise ValueError(
                        "WARNING: only one of the parameter files can be WFSS mode."
                    )
                filter_name = params["Readout"]["filter"]
                pupil_name = params["Readout"]["pupil"]
                dispname = "{}_dispersed_seed_image.fits".format(
                    params["Output"]["file"].split(".fits")[0]
                )
                self.default_dispersed_filename = os.path.join(
                    params["Output"]["directory"], dispname
                )

                bkgd_output_file = "{}_background_image.fits".format(
                    params["Output"]["file"].split(".fits")[0]
                )
                self.background_image_filename = os.path.join(
                    params["Output"]["directory"], bkgd_output_file
                )
                ...
                modified_file = os.path.join(
                    outdir, "tmp_update_to_wfss_mode_{}".format(basename)
                )
        ...
        if wfss_files_found == 0:
            raise ValueError(
                (
                    "WARNING: No WFSS mode parameter files found. One of the parameter "
    ...
    def get_subarr_bounds(self, subname, sdict):
        ...
        if subname in sdict["AperName"]:
            ...
            bounds = [
                sdict["xstart"].data[mtch][0],
                sdict["ystart"].data[mtch][0],
                sdict["xend"].data[mtch][0],
                sdict["yend"].data[mtch][0],
            ]
            return bounds
        else:
            raise ValueError(
                (
                    "WARNING: {} is not a subarray aperture name present "
                    "in the subarray definition file.".format(subname)
                )
            )

    def invalid(self, field, value):
        raise ValueError(("WARNING: invalid value for {}: {}".format(field, value)))
    ...
    def save_dispersed_seed_image(self, seed_image):
        ...
        self.logger.info(
            ("Dispersed seed image saved to {}".format(self.disp_seed_filename))
        )
    ...
    def add_options(self, parser=None, usage=None):
        if parser is None:
            parser = argparse.ArgumentParser(
                usage=usage,
                description=(
                    "Wrapper for the creation of" " WFSS simulated exposures."
                ),
            )
        parser.add_argument(
            "paramfiles",
            help=(
                "List of files describing the input parameters and "
                "instrument settings to use. (YAML format)."
            ),
            nargs="+",
        )
        parser.add_argument(
            "--crossing_filter",
            help=("Name of crossing filter to use in conjunction " "with the grism."),
            default=None,
        )
        parser.add_argument(
            "--module",
            help="NIRCam module to use for simulation. Use 'A' or 'B'",
            default=None,
        )
        parser.add_argument(
            "--direction",
            help=(
                "Direction of dispersion (along rows or along columns). "
                "Use 'R' or 'C'"
            ),
            default=None,
        )
        parser.add_argument(
            "--override_dark",
            help=(
                "If supplied, skip the dark preparation step and "
        ...
        parser.add_argument(
            "--extrapolate_SED",
            help=(
                "If true, the SED created from the filter-averaged "
                "magnitudes will be extrapolated to fill the "
                "wavelngth range of the grism"
            ),
            action="store_true",
        )
        return parser
...
if __name__ == "__main__":
    ...
    usagestring = (
        "USAGE: wfss_simualtor.py file1.yaml file2.yaml --crossing_filter F444W --direction R "
        "--module A"
    )
