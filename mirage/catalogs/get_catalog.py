...
def get_target_coords(xml_filename):
    ...
    for key in targ_dict:
        ...
        target_coords.append(
            SkyCoord("{} {}".format(ra_str, dec_str), unit=(u.hourangle, u.deg))
        )
...
def get_sw_catalog(target_coords, output_directory, search_radius=15 * u.arcmin):
    ...
    for i_obs, t in enumerate(target_coords):
        ...
        catalog_filename_sw = os.path.join(
            output_directory,
            "2MASS_RA{:.2f}deg_Dec{:.2f}deg.list".format(t.ra.degree, t.dec.degree),
        )
        ...
        if os.path.isfile(catalog_filename_sw):
            ...
            if catalog_filename_sw not in catalog_filenames_sw[:i_obs]:
                print(
                    "Shortwave catalog file {} already exists. Will not overwrite.".format(
                        os.path.basename(catalog_filename_sw)
                    )
                )
            continue
        ...
        print(
            "Queried {} 2MASS objects within {} {} of RA, Dec ({:.2f}, {:.2f}).".format(
                len(queried_catalog_sw),
                search_radius.value,
                str(search_radius.unit),
                t.ra.deg,
                t.dec.deg,
            )
        )
...
def get_catalog(target_coords, output_directory, search_radius=15 * u.arcmin):
    ...
    for i_obs, t in enumerate(target_coords):
        ...
        catalog_filename = os.path.join(
            output_directory,
            "WISE_and_2MASS_RA{:.2f}deg_Dec{:.2f}deg.list".format(
                t.ra.degree, t.dec.degree
            ),
        )
        ...
        if os.path.isfile(catalog_filename):
            ...
            if catalog_filename not in catalog_filenames[:i_obs]:
                print(
                    "Longwave catalog file {} already exists. Will not overwrite.".format(
                        os.path.basename(catalog_filename)
                    )
                )
            continue
        ...
        v = Vizier(
            catalog="II/328/allwise", columns=["RAJ2000", "DEJ2000", "W2mag", "Kmag"]
        )
        v.ROW_LIMIT = -1
        result = v.query_region(t, radius=search_radius)
        ...
        print(
            "Queried {} WISE objects within {} {} of RA, Dec ({:.2f}, {:.2f}).".format(
                len(queried_catalog),
                search_radius.value,
                str(search_radius.unit),
                t.ra.deg,
        ...
        queried_catalog["Kmag"].fill_value = 99.0
        queried_catalog["W2mag"].fill_value = 99.0
        ...
        f460m = Column(
            queried_catalog["nircam_f480m_magnitude"].data,
            name="nircam_f460m_magnitude",
        )
