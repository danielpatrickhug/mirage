...
from mirage.utils.constants import (
    IMAGING_ALLOWED_CATALOGS,
    WFSS_ALLOWED_CATALOGS,
    TS_IMAGING_ALLOWED_CATALOGS,
    TS_GRISM_ALLOWED_CATALOGS,
)
    ...
    for catalog in catalogs:
        ...
        if "index" in cat.colnames:
            ...
            if min_val == 0:
                raise ValueError(
                    (
                        "{} has a source with an index value of 0. This is not allowed. "
                        "Zero is reserved for pixels in the segmentation map with no source flux.".format(
                            catalog
                        )
                    )
                )
            if min_val < 0:
                raise ValueError(
                    "{} has sources with negative index numbers. This is not allowed.".format(
                        catalog
                    )
                )
            ...
            raise ValueError(
                "{} does not have an 'index' column. Cannot compare to other catalogs.".format(
                    catalog
                )
            )
...
def get_nonsidereal_catalog_name(cat_dict, target_name, instrument_name):
    ...
    if "moving_target_to_track" in target_cat.keys():
        if not os.path.isfile(target_cat["moving_target_to_track"]):
            raise ValueError(
                (
                    "{} is listed as the non-sidereal target catalog for "
                    "{}, but it appears this file does not exist.".format(
                        target_cat["moving_target_to_track"], target_name
                    )
                )
            )
        else:
        ...
        if instrument_name not in target_cat.keys():
            raise ValueError(
                (
                    "Catalog dictionary does not contain a 'moving_target_to_track' catalog "
            ...
            if "moving_target_to_track" in target_cat[instrument_name].keys():
                if not os.path.isfile(
                    target_cat[instrument_name]["moving_target_to_track"]
                ):
                    raise ValueError(
                        (
                            "{} is listed as the non-sidereal target catalog for "
