...
class UWS:
    ...
    def runJob(self, idJob):
        r = requests.post(
            self._url + "/jobs/" + str(idJob) + "/phase",
            auth=self._auth,
            data={"PHASE": "RUN"},
        )
    ...
    def maxDurationJob(self, idJob, seconds):
        r = requests.post(
            self._url + "/jobs/" + str(idJob) + "/executionduration",
            auth=self._auth,
            data={"EXECUTIONDURATION": seconds},
        )
        return self._getJobInfosFromReqResult(r, "setting execution duration of a job")

    def timeDestructJob(self, idJob, timeStr):
        r = requests.post(
            self._url + "/jobs/" + str(idJob) + "/destruction",
            auth=self._auth,
            data={"DESTRUCTION": timeStr},
        )
        return self._getJobInfosFromReqResult(r, "setting execution duration of a job")
    ...
    def setParams(self, idJob, params):
        ...
        for p in params:
            data[p[0]] = p[1]
        r = requests.post(
            self._url + "/jobs/" + str(idJob) + "/parameters",
            auth=self._auth,
            data=data,
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Makes requests to web services, either the Besancon Model of the Galaxy or Gravpot web service",
        add_help=False,
    )

    serverGrp = parser.add_argument_group(
        "Server options", "Options for server connection."
    )
    serverGrp.add_argument(
        "--url", help="URL of the server.", default="https://model.obs-besancon.fr/ws"
    )
    serverGrp.add_argument(
        "--user",
        help="Username used to connect to server, by default it is the system username ("
        + os.getlogin()
        + ").",
        default=os.getlogin(),
    )
    serverGrp.add_argument(
        "--pass",
        dest="passwd",
        help='Password used to connect to server, by default the password is prompted. Usage of this option is discouraged because it will be visible, for example, with "ps" command and written to history.',
    )

    jobGrp = parser.add_argument_group(
        "Job selection",
        'Select the job you want to manipulate. To create a new one type "--create". If this option is not present, it returns your list of jobs and exit ignoring options in "Actions".',
    ).add_mutually_exclusive_group()
    jobGrp.add_argument("-c", "--create", help="Create a new job.", action="store_true")
    jobGrp.add_argument(
        "-j", "--job", type=int, help="The job affected by this request."
    )

    actionGrp = parser.add_argument_group(
        "Actions", "Configure and make actions on the selected job."
    )
    actionGrp.add_argument(
        "-p",
        "--param",
        help="Set parameter named PARAMNAME with VALUE.",
        action="append",
        nargs=2,
    ...
    actionGrp.add_argument(
        "--execdur",
        help="Set maximum execution duration of a job in seconds (this setting can be overridden by server's configuration).",
        type=int,
    )
    actionGrp.add_argument(
        "--tdest",
        help="Set destruction time of a job (date+time in ISO8601 format, this setting can be overridden by server's configuration).",
    )
    actionGrp.add_argument(
        "--run", help="Send job for computation.", action="store_true"
    )
    ...
    if args.create:
        ...
        print(
            " JobID        phase       startTime          endTime            destruction    "
        )
        print(
            "----------   ---------   ----------------   ----------------   ----------------"
        )
        ...
        for job in jobs.findall("uws:job", XML_NS):
            ...
            print(
                "%10d   %9s   %16s   %16s   %16s"
                % (int(jobId), phase, startTime, endTime, destruction)
            )
        exit(0)
    ...
    print(
        "executionDuration :"
        + xmlTextIfFound(job.find("uws:executionDuration", XML_NS))
    )
    ...
    if results is not None:
        ...
        for res in results.findall("uws:result", XML_NS):
            print(
                "{:<18}:{}".format(
                    res.get("id"), res.get("{http://www.w3.org/1999/xlink}href")
                )
            )
