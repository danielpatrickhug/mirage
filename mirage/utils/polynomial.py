...
def jacob(a, b, x, y, order):
    ...
    j = dpdx(a, x, y, order) * dpdy(b, x, y, order) - dpdx(b, x, y, order) * dpdy(
        a, x, y, order
    )
    j = scipy.fabs(j)
    return j
...
def invert(a, b, u, v, n, verbose=False):
    ...
    while err > tol:
        ...
        j = scipy.array(
            [[dpdx(a, x, y, n), dpdy(a, x, y, n)], [dpdx(b, x, y, n), dpdy(b, x, y, n)]]
        )
        ...
        for n in range(m + 1):
            for mu in range(0, m - n + 1):
                for j in range(m - n - mu, m - mu + 1):
                    factor = (
                        (-1) ** (m - n - mu) * choose(m - j, mu) * choose(j, m - n - mu)
                    )
                ...
                for beta in range(i - j - alpha + 1):
                    ...
                    for gamma in range(j + 1):
                        for delta in range(j - gamma + 1):
                            f2 = (
                                choose(j, gamma)
                                * choose(j - gamma, delta)
                                * b[0] ** (j - gamma - delta)
                                * b[1] ** gamma
                                * b[2] ** delta
                            )
                            A2[alpha + beta + gamma + delta, beta + delta] += (
                                A[k] * f1 * f2
                            )
                            B2[alpha + beta + gamma + delta, beta + delta] += (
                                B[k] * f1 * f2
                            )
            k += 1
