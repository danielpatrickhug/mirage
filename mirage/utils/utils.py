    ...
    for key in new_dictionary.keys():
        ...
        if len(new_dictionary[key]) == 0:
            ...
            if braid:
                ...
                new_dictionary[key] = [
                    sub[i]
                    for i in range(len(added_dictionary[key]))
                    for sub in [new_dictionary[key], added_dictionary[key]]
                ]
            else:
...
def calc_frame_time(instrument, aperture, xdim, ydim, amps):
    ...
    return ((1.0 * xdim / amps + colpad) * (ydim + rowpad) + fullpad) * 1.0e-5
...
def countrate_to_magnitude(
    ...
    # two factors of 0.84
    if (instrument.lower() == "niriss") and (
        filter_name.upper() in NIRISS_FILTER_WHEEL_FILTERS
    ):
        count_scale = 1.0 / 0.84
    else:
        count_scale = 1.0
    ...
    if magsys.lower() == "abmag":
        try:
            ...
            raise ValueError(
                (
                    "Count rate to AB mag conversion failed."
    ...
    if magsys.lower() == "vegamag":
        try:
            ...
            raise ValueError(
                (
                    "Count rate to Vega mag conversion failed."
    ...
    if magsys.lower() == "stmag":
        try:
            ...
            raise ValueError(
                (
                    "Count rate to ST mag conversion failed."
...
def crop_to_subarray(data, bounds):
    ...
    if all(valid):
        ...
        raise ValueError(
            (
                "WARNING: subarray bounds are outside the "
    ...
    if not offline:
        if not os.path.isdir(variable_directory):
            raise FileNotFoundError(
                (
                    "The directory contained in the {} "
        ...
        for key2 in pathdict[key1]:
            ...
            if key2 not in params[key1].keys():
                if key2 in CRDS_FILE_TYPES.keys():
                    logger.info(
                        '{}:{} field not present in input. Setting equal to "crds"'.format(
                            key1, key2
                        )
                    )
            ...
            if params[key1][key2].lower() not in ["none", "config", "crds"]:
                ...
                key2 in CRDS_FILE_TYPES.keys() and params[key1][key2].lower() == "crds"
            ):
                ...
                if (
                    (params["Inst"]["instrument"].lower() == "nircam")
                ...
                if key2 != "transmission":
                    mapping = crds_tools.get_reffiles(
                        query_dictionary, [CRDS_FILE_TYPES[key2]], download=not offline
                    )
                    ...
                    logger.info(
                        "From grism library, found {} as the POM transmission file.".format(
                            params[key1][key2]
                        )
                    )
                ...
                if key2 == "ipc":
                    ...
                    inverted_kernel_file = os.path.join(
                        ipc_path, "Kernel_to_add_IPC_effects_from_{}".format(ipc_file)
                    )
                    # Check to see if the version of the IPC file with an
                    # inverted kernel already exists.
                    if os.path.isfile(inverted_kernel_file):
                        logger.info(
                            "Found an existing inverted kernel for this IPC file: {}".format(
                                inverted_kernel_file
                            )
                        )
...
def get_lw_grism_tso_intermeidate_aperture(aperture):
    ...
    if lw_filter in ["F277W", "F322W2", "F356W"]:
        ...
        raise ValueError(
            "Unrecognized Grism TSO LW filter: {}. What intermediate aperture is used?".format(
                lw_filter
            )
        )
...
def get_filter_throughput_file(
    instrument, filter_name, pupil_name, nircam_module=None, fgs_detector=None
):
    ...
    if instrument == "nircam":
        ...
        tp_file = "{}_{}_nircam_plus_ote_throughput_mod{}_sorted.txt".format(
            filter_name.upper(), pupil_name.upper(), nircam_module.lower()
        )
    elif instrument == "niriss":
        element = filter_name
    ...
    if not os.path.isfile(throughput_file):
        raise FileNotFoundError(
            "Throughput file {} does not exist.".format(throughput_file)
        )
...
def get_subarray_info(params, subarray_table):
    ...
    if array_name in subarray_table["AperName"]:
        ...
        if namps != 0:
            ...
            try:
                if (params["Readout"]["namp"] == 1) or (params["Readout"]["namp"] == 4):
                    logger.info(
                        (
                            "CAUTION: Aperture {} can be used with either "
                            "a 1-amp or a 4-amp readout. The difference is a factor of 4 in "
                            "readout time. You have requested {} amps.".format(
                                subarray_table["AperName"].data[mtch][0],
                                params["Readout"]["namp"],
                            )
                        )
                    )
                    ...
                    raise ValueError(
                        (
                            "WARNING: {} requires the number of amps to be 1 or 4. Please set "
                            "'Readout':'namp' in the input yaml file to one of these values.".format(
                                array_name
                            )
                        )
                    )
                ...
                raise KeyError(
                    (
                        "WARNING: 'Readout':'namp' not present in input yaml file. "
        ...
        raise ValueError(
            (
                "WARNING: subarray name {} not found in the "
                "subarray dictionary {}.".format(
                    array_name, params["Reffiles"]["subarray_defs"]
                )
            )
        )
    return params
...
def magnitude_to_countrate(
    ...
    if magsys.lower() == "abmag":
        try:
            ...
            raise ValueError(
                (
                    "AB mag to countrate conversion failed."
    ...
    if magsys.lower() == "vegamag":
        try:
            ...
            raise ValueError(
                (
                    "Vega mag to countrate conversion failed."
    ...
    if magsys.lower() == "stmag":
        try:
            ...
            raise ValueError(
                (
                    "ST mag to countrate conversion failed."
...
def make_mag_column_names(instrument, filters):
    ...
    for filter_name in filters:
        ...
        if filter_name.upper() not in possible_filters:
            raise ValueError(
                "{} is not recognized or not supported for {}.".format(
        ...
        if instrument in ["niriss", "fgs"]:
            ...
            if len(parts) == 2:
                if parts[0] in pupil_wheel_options:
                    ...
                    pup_val = parts[1]
                    filt_val = parts[0]
                column_name = "{}_{}_{}_magnitude".format(
                    instrument, filt_val.lower(), pup_val.lower()
                )
            else:
                raise ValueError(
                    "{} is an incomplete filter name. Filter and pupil must be present.".format(
                        filter_name
                    )
                )
...
def organize_config_files(offline=False):
    ...
    for instrument in "niriss fgs nircam miri nirspec".split():
        ...
        if instrument in "niriss fgs nircam".split():
            config_info["global_subarray_definitions"][instrument] = asc.read(
                os.path.join(modpath, "config", subarray_def_file)
            )
            config_info["global_readout_patterns"][instrument] = asc.read(
                os.path.join(modpath, "config", readout_pattern_file)
            )
        config_info["global_subarray_definition_files"][instrument] = os.path.join(
            modpath, "config", subarray_def_file
        )
        config_info["global_readout_pattern_files"][instrument] = os.path.join(
            modpath, "config", readout_pattern_file
        )
        config_info["global_crosstalk_files"][instrument] = os.path.join(
            modpath, "config", crosstalk_file
        )
        config_info["global_filtpupilcombo_files"][instrument] = os.path.join(
            modpath, "config", filtpupilcombo_file
        )
        config_info["global_filter_position_files"][instrument] = os.path.join(
            modpath, "config", filter_position_file
        )
        config_info["global_flux_cal_files"][instrument] = os.path.join(
            modpath, "config", flux_cal_file
        )
        config_info["global_psf_wing_threshold_file"][instrument] = os.path.join(
            modpath, "config", psf_wing_threshold_file
        )
...
def parse_RA_Dec(ra_string, dec_string):
    ...
    try:
        ...
        ra_degrees = 15.0 * (
            int(values[0]) + int(values[1]) / 60.0 + float(values[2]) / 3600.0
        )
        ...
        raise ValueError(
            "Error parsing RA, Dec strings: {} {}".format(ra_string, dec_string)
        )
...
def read_subarray_definition_file(filename):
    ...
    try:
        ...
        raise RuntimeError(
            ("Error: could not read in subarray definitions file: {}".format(filename))
        )
    return data
...
def standardize_filters(instrument, filter_values):
    ...
    if instrument.lower() == "fgs":
        ...
        pw_values = NIRCAM_PUPIL_WHEEL_FILTERS + [
            "CLEAR",
            "GRISMR",
            "GRISMC",
            "GDHS0",
        ...
        reverse_nircam_2_filter_crosses = [
            "{}/{}".format(ele.split("/")[1], ele.split("/")[0])
            for ele in NIRCAM_2_FILTER_CROSSES
        ]
        wlp8_combinations = [
            "{}/WLP8".format(ele) for ele in NIRCAM_WL8_CROSSING_FILTERS
        ]
        wlm8_combinations = [
            "{}/WLM8".format(ele) for ele in NIRCAM_WL8_CROSSING_FILTERS
        ]
        wl8_combinations = wlp8_combinations + wlm8_combinations
        reverse_wlp8 = [
            "{}/{}".format(ele.split("/")[1], ele.split("/")[0])
            for ele in wlp8_combinations
        ]
        reverse_wlm8 = [
            "{}/{}".format(ele.split("/")[1], ele.split("/")[0])
            for ele in wlm8_combinations
        ]
        ...
        for filter_value in filter_values:
            ...
            if len(parts) == 2:
                if filter_value in NIRCAM_2_FILTER_CROSSES + wl8_combinations:
                    ...
                    filter_value
                    in reverse_nircam_2_filter_crosses + reverse_wl8_combinations
                ):
                    ...
                    if parts[1] not in pw_values:
                        ...
                        raise ValueError(
                            "Both {} and {} are in the pupil wheel. This is not supported.".format(
                                parts[0], parts[1]
                            )
                        )
                    ...
                    if parts[1] not in pw_values:
                        raise ValueError(
                            "Both {} and {} are in the filter wheel. This is not supported.".format(
                                parts[0], parts[1]
                            )
                        )
                    else:
                ...
                if filter_value in NIRCAM_CLEAR_CROSSING_FILTERS:
                    ...
                    raise ValueError(
                        "Unsupported shorthand filter value: {}".format(filter_value)
                    )
            ...
            if len(parts) == 1:
                ...
                if parts[0] in ["CLEAR", "CLEARP"]:
                    ...
                    if parts[1] not in ["CLEAR", "CLEARP"]:
                        raise ValueError(
                            (
                                "{} unsupported. For NIRISS, filter must be CLEAR or pupil must be CLEARP.".format(
                                    filter_value
                                )
                            )
                        )
                ...
                raise ValueError(
                    "{} is an unsupported filter value for NIRISS".format(filter_value)
                )
    else:
