...
class SimInput:
    def __init__(
        ...
        segmap_flux_limit=None,
        segmap_flux_limit_units=None,
        add_ghosts=True,
        convolve_ghosts_with_psf=False,
        convolve_extended_with_psf=True,
        offline=False,
    ):
        ...
        if (segmap_flux_limit is not None) and (segmap_flux_limit_units is None):
            raise ValueError(
                "If segmap_flux_limit is provided, segmap_flux_units must also be provided."
            )
        ...
        if input_xml is not None:
            if self.observation_list_file is None:
                self.observation_list_file = os.path.join(
                    self.output_dir, "observation_list.yaml"
                )
            self.apt_xml_dict, self.xml_skipped_observations = get_observation_dict(
                self.input_xml,
                self.observation_list_file,
                catalogs,
            ...
            self.logger.error(
                "No input xml file provided. Observation dictionary not constructed."
            )
    ...
    def add_catalogs(self):
        ...
        for i in range(n_exposures):
            ...
            if self.point_source[i] is not None:
                ...
                self.info["point_source"][i] = os.path.abspath(
                    os.path.expandvars(
            ...
            if self.galaxyListFile[i] is not None:
                self.info["galaxyListFile"][i] = os.path.abspath(
                    os.path.expandvars(
            ...
            if self.extended[i] is not None:
                self.info["extended"][i] = os.path.abspath(
                    os.path.expandvars(
            ...
            if self.movingTarg[i] is not None:
                self.info["movingTarg"][i] = os.path.abspath(
                    os.path.expandvars(
                        self.catalog_match(
                            filt, pup, self.movingTarg, "moving point source target"
                        )
                    )
                )
            else:
            ...
            if self.movingTargSersic[i] is not None:
                self.info["movingTargSersic"][i] = os.path.abspath(
                    os.path.expandvars(
                        self.catalog_match(
                            filt, pup, self.movingTargSersic, "moving sersic target"
                        )
                    )
                )
            else:
            ...
            if self.movingTargExtended[i] is not None:
                self.info["movingTargExtended"][i] = os.path.abspath(
                    os.path.expandvars(
                        self.catalog_match(
                            filt, pup, self.movingTargExtended, "moving extended target"
                        )
                    )
                )
            else:
            ...
            if self.movingTargToTrack[i] is not None:
                self.info["movingTargToTrack"][i] = os.path.abspath(
                    os.path.expandvars(
                        self.catalog_match(
                            filt,
                            pup,
                            self.movingTargToTrack,
                            "non-sidereal moving target",
                        )
                    )
                )
            else:
    ...
    def add_crds_reffile_names(self):
        ...
        for status in unique_obs_info:
            ...
            reffiles = crds_tools.get_reffiles(
                status_dict, files_no_transmission, download=not self.offline
            )
            ...
            if "transmission" not in reffiles.keys():
                ...
                self.logger.info(
                    "Using transmission file: {}".format(reffiles["transmission"])
                )
            ...
            inverted_file, must_invert = SimInput.inverted_ipc_kernel_check(
                reffiles["ipc"]
            )
            if not must_invert:
    ...
    def create_inputs(self):
        ...
        if (
            ...
            final_file = os.path.join(
                self.output_dir,
                "Observation_table_for_" + infile + "_with_yaml_parameters.csv",
            )
            ...
            for instrument_name in np.unique(self.info["Instrument"]):
                siaf_dictionary[instrument_name] = siaf_interface.get_instance(
                    instrument_name
                )
            ...
            raise FileNotFoundError(
                (
                    "WARNING. You must include either an ascii table file of observations"
        ...
        # exposure itself, as well.)
            ...
            raise ValueError(
                (
                    "self.reffile_defaults is not equal to 'crds' "
        ...
        darks = []
        lindarks = []
        for instrument, det in zip(
            [s.lower() for s in self.info["Instrument"]], self.info["detector"]
        ):
        ...
        if self.use_linearized_darks:
            ...
            if set(lindarks) == set([None]):
                raise RuntimeError(
                    (
                        "ERROR: Linearized darks requested, but no linearized dark files "
                        "found. Check: {}"
                    ).format(
                        os.path.join(os.path.expandvars("$MIRAGE_DATA"), instrument)
                    )
                )
        else:
            ...
            if set(darks) == set([None]):
                raise RuntimeError(
                    (
                        "ERROR: Raw darks requested, but no raw dark files found. "
                        "Check: {}"
                    ).format(
                        os.path.join(os.path.expandvars("$MIRAGE_DATA"), instrument)
                    )
        ...
        for i in range(len(self.info["Mode"])):
            if self.info["Mode"][i].lower() in ["wfss", "ts_grism"]:
                if (
                    ...
                    or self.info["detector"][i][-1] == "5"
                ):
                    grism_source_image[i] = "True"
                    grism_input_only[i] = "True"
                # SW detectors shouldn't be wfss
                if (
                    self.info["Instrument"][i] == "NIRCAM"
                    and self.info["detector"][i][-1] != "5"
                ):
        ...
        self.logger.info(
            "Updated observation table file saved to {}".format(final_file)
        )
        ...
        for i, instrument in enumerate(self.info["Instrument"]):
            ...
            if instrument not in "fgs nircam niriss".split():
                ...
                if self.info["APTTemplate"][i] == "NircamCoron":
                    ...
                    if (
                        self.info["LongPupil"][i] == "n/a"
                        and self.info["detector"][i] == "A5"
                    ) or (
                        self.info["ShortPupil"][i] == "n/a"
                        and self.info["detector"][i] in ["A1", "A2", "A3", "A4"]
                    ):
                        print(
                            f"Skipping {self.info['yamlfile'][i]} because this coronagraphy obs does not use that detector"
                        )
                        continue

                    # SW coronagraphy exposures that use MASKRND collect data from A2 only
                    if (
                        self.info["ShortPupil"][i] == "MASKRND"
                        and self.info["detector"][i] != "A2"
                    ):
                        print(
                            f"Skipping yaml for {self.info['detector'][i]} with {self.info['ShortPupil'][i]}"
                        )
                        continue

                    # SW coronagraphy exposures that use MASKSWB collect data from A4 only
                    if (
                        self.info["ShortPupil"][i] == "MASKSWB"
                        and self.info["detector"][i] != "A4"
                    ):
                        print(
                            f"Skipping yaml for {self.info['detector'][i]} with {self.info['ShortPupil'][i]}"
                        )
            ...
            if (
                isinstance(file_dict["SubpixelPositions"], str)
                and file_dict["SubpixelPositions"].upper() == "NONE"
            ):
                subpixtot = 1
            else:
                try:
            ...
            primary_dither = np.ceil(1.0 * tot_dith / subpixtot)
            ...
            file_dict["subarray_def_file"] = self.config_information[
                "global_subarray_definition_files"
            ][instrument]
            file_dict["readpatt_def_file"] = self.config_information[
                "global_readout_pattern_files"
            ][instrument]
            file_dict["crosstalk_file"] = self.config_information[
                "global_crosstalk_files"
            ][instrument]
            file_dict["filtpupilcombo_file"] = self.config_information[
                "global_filtpupilcombo_files"
            ][instrument]
            file_dict["filter_position_file"] = self.config_information[
                "global_filter_position_files"
            ][instrument]
            file_dict["flux_cal_file"] = self.config_information[
                "global_flux_cal_files"
            ][instrument]
            file_dict["psf_wing_threshold_file"] = self.config_information[
        ...
        for obs in obs_ids:
            ...
            activity_list = list(
                set([vf[17:29] for vf in self.info["yamlfile"] if vf[7:10] == obs])
            )
            n_activities = len(activity_list)
            exposure_list = list(
                set([vf[20:25] for vf in self.info["yamlfile"] if vf[7:10] == obs])
            )
            ...
            if coord_par:
                ...
                if len(par_indexes) > 0:
                    parallel_instrument = self.info["Instrument"][
                        obs_indexes[par_indexes[0]]
                    ]
                else:
            ...
            if prime_instrument.upper() == "NIRCAM":
                ...
                detectors_used = np.array(self.info["detector"])[
                    obs_indexes[pri_indexes]
                ]
            elif parallel_instrument.upper() == "NIRCAM":
                module = self.info["Module"][obs_indexes[par_indexes[0]]]
                detectors_used = np.array(self.info["detector"])[
                    obs_indexes[par_indexes]
                ]

            if (prime_instrument.upper() == "NIRCAM") or (
                parallel_instrument.upper() == "NIRCAM"
            ):
                if module == "ALL":
                    module = "A and B"
            ...
            if coord_par == "true":
                instrument_string = "    Prime: {}, Parallel: {}".format(
                    prime_instrument, parallel_instrument
                )
            else:
            ...
            if (prime_instrument.upper() == "NIRCAM") or (
                ...
                self.logger.info(
                    "    {} NIRCam detector(s) in module {}".format(n_det, module)
                )
        ...
        logging_functions.move_logfile_to_standard_location(
            self.input_xml,
            STANDARD_LOGFILE_NAME,
            yaml_outdir=log_outdir,
            log_type="yaml_generator",
        )
    ...
    def create_output_name(self, input_obj, index=0):
        ...
        base = "jw{}{}{}_{}{}{}_{}_".format(
            proposal_id,
            observation,
            visit_number,
            visit_group,
            parallel_sequence_id,
            activity_id,
            exposure,
        )
        return base
    ...
    def get_reffile(self, refs, detector):
        ...
        for key in refs:
            if detector in key:
                return refs[key]
        self.logger.error(
            "WARNING: no file found for detector {} in {}".format(detector, refs)
        )
    ...
    def inverted_ipc_kernel_check(filename):
        ...
        inverted_name = os.path.join(
            dirname, "Kernel_to_add_IPC_effects_from_{}".format(basename)
        )
            ...
            for i in range(len(self.info["Module"])):
                ...
                yaml_names.append(
                    os.path.abspath(
                        os.path.join(
                            self.output_dir,
                            "Act{}_{}_{}_Dither{}.yaml".format(act, det, mode, dither),
                        )
                    )
                )
                fits_names.append(
                    "Act{}_{}_{}_Dither{}_uncal.fits".format(act, det, mode, dither)
                )
    ...
    def nonsidereal_pointing_updates(self):
        ...
        inst_targs = [
            [t, i] for t, i in zip(all_nonsidereal_targs, all_nonsidereal_instruments)
        ]
        ...
        for element in unique:
            ...
            for obs_name in obs_this_target:
                ...
                if "ephemeris_file" in catalog_table.colnames:
                    all_times = [
                        ephemeris_tools.to_timestamp(elem) for elem in start_dates
                    ]
                    ...
                    try:
                        ...
                        raise ValueError(
                            (
                                "Observation dates ({} - {}) are not present within the ephemeris file {}".format(
                                    start_dates[0], start_dates[-1], ephemeris_file
                                )
                            )
                        )
                else:
                    if not pos_in_xy:
                        ...
                        base_ra, base_dec = parse_RA_Dec(
                            catalog_table["x_or_RA"].data[0],
                            ...
                            if len(unique_apertures) > 1:
                                if inst.lower() == "nircam":
                                    ...
                                    if np.min(det_ints) < 5 and np.max(det_ints) == 5:
                                        raise ValueError(
                                            (
                                                "Non-sidereal source {} has no ephemeris file, and a velocity that "
                                                ...
                                                "when placing the source.".format(
                                                    targ, obs_name
                                                )
                                            )
                                        )
                        ...
                        for ob_date in start_dates[1:]:
                            ...
                            delta_ra = ra_vel * delta_time.total_seconds() / 3600.0
                            delta_dec = dec_vel * delta_time.total_seconds() / 3600.0
                        ...
                        if len(unique_apertures) > 1:
                            raise ValueError(
                                (
                                    "Non-sidereal source {} has no ephemeris file, and a location that "
                                    ...
                                    "to use when placing the source.".format(
                                        targ, obs_name
                                    )
                                )
                            )
                        ...
                        # to RA, Dec
                        base_ra, base_dec = aperture_xy_to_radec(
                            catalog_table["x_or_RA"].data[0],
                            catalog_table["y_or_Dec"].data[0],
                            inst,
                            aperture,
    ...
    def set_global_definitions(self):
        ...
        for instrument in "niriss fgs nircam miri nirspec".split():
            ...
            if instrument in "niriss fgs nircam".split():
                self.global_subarray_definitions[instrument] = self.get_subarray_defs(
                    filename=os.path.join(self.modpath, "config", subarray_def_file)
                )
                self.global_readout_patterns[instrument] = self.get_readpattern_defs(
                    filename=os.path.join(self.modpath, "config", readout_pattern_file)
                )
            self.global_subarray_definition_files[instrument] = os.path.join(
                self.modpath, "config", subarray_def_file
            )
            self.global_readout_pattern_files[instrument] = os.path.join(
                self.modpath, "config", readout_pattern_file
            )
            self.global_crosstalk_files[instrument] = os.path.join(
                self.modpath, "config", crosstalk_file
            )
            self.global_filtpupilcombo_files[instrument] = os.path.join(
                self.modpath, "config", filtpupilcombo_file
            )
            self.global_filter_position_files[instrument] = os.path.join(
                self.modpath, "config", filter_position_file
            )
            self.global_flux_cal_files[instrument] = os.path.join(
                self.modpath, "config", flux_cal_file
            )
            self.global_psf_wing_threshold_file[instrument] = os.path.join(
                self.modpath, "config", psf_wing_threshold_file
            )
    ...
    def multiple_catalog_match(self, filter, cattype, matchlist):
        ...
        """
        self.logger.warning(
            "WARNING: multiple {} catalogs matched! Using the first.".format(cattype)
        )
    ...
    def no_catalog_match(self, filter, cattype):
        ...
        self.logger.warning(
            "Using the first input for now. Make sure input catalog names have"
        )
        self.logger.warning(
            "the appropriate filter name in the filename to get matching to work."
        )
    ...
    def path_defs(self):
        ...
        self.simdata_output_dir = os.path.abspath(
            os.path.expandvars(self.simdata_output_dir)
        )
        ...
        if self.observation_list_file is not None:
            self.observation_list_file = os.path.abspath(
                os.path.expandvars(self.observation_list_file)
            )
    ...
    def reffile_setup(self):
        ...
        for instrument in "nircam niriss fgs".split():
            ...
            self.psfpath[instrument] = os.path.join(
                self.datadir, instrument, "gridded_psf_library"
            )
            self.psfbasename[instrument] = instrument
            self.reference_file_dir[instrument] = os.path.join(
                self.datadir, instrument, "reference_files"
            )
            ...
            self.configfiles[instrument]["filter_throughput"] = os.path.join(
                self.modpath, "config", "placeholder.txt"
            )
        ...
        for instrument in "nircam niriss fgs miri nirspec".split():
            ...
            if self.offline:
                ...
                for det in self.det_list[instrument]:
                    self.dark_list[instrument][det] = glob(
                        os.path.join(rawdark_dir, det, "*.fits")
                    )
                    self.lindark_list[instrument][det] = glob(
                        os.path.join(lindark_dir, det, "*.fits")
                    )

            elif instrument in ["nirspec", "miri"]:
                for key in (
                    "subarray_def_file fluxcal filtpupil_pairs readpatt_def_file crosstalk "
                ...
                for det in self.det_list[instrument]:
                    if det == "G1":
                        self.dark_list[instrument][det] = glob(
                            os.path.join(
                                self.datadir, "fgs/darks/raw", FGS1_DARK_SEARCH_STRING
                            )
                        )
                        self.lindark_list[instrument][det] = glob(
                            os.path.join(
                                self.datadir,
                                "fgs/darks/linearized",
                                FGS1_DARK_SEARCH_STRING,
                            )
                        )
                        ...
                        self.dark_list[instrument][det] = glob(
                            os.path.join(
                                self.datadir, "fgs/darks/raw", FGS2_DARK_SEARCH_STRING
                            )
                        )
                        self.lindark_list[instrument][det] = glob(
                            os.path.join(
                        ...
                        self.dark_list[instrument][det] = glob(
                            os.path.join(
                                self.datadir, "niriss/darks/raw", "*uncal.fits"
                            )
                        )
                        self.lindark_list[instrument][det] = glob(
                            os.path.join(
                                self.datadir,
                                "niriss/darks/linearized",
    ...
    def get_psf_path(self):
        ...
        if self.psf_paths is None:
            self.logger.info(
                "No PSF path provided. Using default path as PSF path for all yamls."
            )
            paths_out = []
            for instrument in self.info["Instrument"]:
                default_path = self.config_information["global_psfpath"][
                    instrument.lower()
                ]
            ...
            raise ValueError(
                "Invalid PSF paths parameter provided. Please "
                "provide the psf_paths in the form of a list of "
                "strings with a length equal to the number of "
                "activities in the APT program ({}), not equal to {}.".format(
                    n_activities, len(self.psf_paths)
                )
            )
        ...
        elif not isinstance(self.psf_paths, list) or not isinstance(
            self.psf_paths, str
        ):
            raise TypeError(
                "Invalid PSF paths parameter provided. Please "
    ...
    def reffiles_from_dict(self, obs_params):
        ...
        try:
            if instrument in ["nircam", "fgs"]:
                files["superbias"] = self.reffile_overrides[instrument]["superbias"][
                    detector
                ][readpattern]
            elif instrument == "niriss":
                files["superbias"] = self.reffile_overrides[instrument]["superbias"][
        ...
        try:
            if instrument == "nircam":
                files["distortion"] = self.reffile_overrides[instrument]["distortion"][
                    detector
                ][filtername][exptype]
            elif instrument == "niriss":
                files["distortion"] = self.reffile_overrides[instrument]["distortion"][
        ...
        try:
            if instrument == "nircam":
                files["transmission"] = self.reffile_overrides[instrument][
                    "transmission"
                ][detector][filtername][pupilname]
            elif instrument == "niriss":
                files["transmission"] = self.reffile_overrides[instrument][
                    "transmission"
                ][filtername][pupilname]
            elif instrument == "fgs":
                files["transmission"] = self.reffile_overrides[instrument][
                    "transmission"
                ][detector]
        ...
        try:
            if instrument == "nircam":
                files["pixelflat"] = self.reffile_overrides[instrument]["pixelflat"][
                    detector
                ][filtername][pupilname]
            elif instrument == "niriss":
                files["pixelflat"] = self.reffile_overrides[instrument]["pixelflat"][
    ...
    def write_yaml(self, input):
        ...
        with open(yamlout, "w") as f:
            ...
            f.write(
                "  mode: {}                # Observation mode (e.g. imaging, WFSS)\n".format(
                    input["Mode"]
                )
            )
            f.write(
                "  use_JWST_pipeline: {}   # Use pipeline in data transformations\n".format(
                    input["use_JWST_pipeline"]
                )
            )
            f.write("\n")
            f.write("Readout:\n")
            f.write(
                "  readpatt: {}        # Readout pattern (RAPID, BRIGHT2, etc) overrides nframe, nskip unless it is not recognized\n".format(
                    input["ReadoutPattern"]
                )
            )
            f.write(
                "  ngroup: {}              # Number of groups in integration\n".format(
                    input["Groups"]
                )
            )
            f.write(
                "  nint: {}          # Number of integrations per exposure\n".format(
                    input["Integrations"]
                )
            )
            f.write(
                "  namp: {}          # Number of amplifiers used to read out detector\n".format(
                    input["namp"]
                )
            )
            f.write(
                "  resets_bet_ints: {} #Number of detector resets between integrations\n".format(
                    self.resets_bet_ints
                )
            )
            ...
            if instrument.lower() == "nircam":
                ...
                if (
                    "NRCA3_DHSPIL" in input["aperture"]
                    or "NRCB4_DHSPIL" in input["aperture"]
                ):  # in ['NRCA3_DHSPIL', 'NRCB4_DHSPIL']:
            ...
            f.write(
                "  array_name: {}    # Name of array (FULL, SUB160, SUB64P, etc)\n".format(
                    full_ap
                )
            )
            f.write(
                "  intermediate_aperture: {}   # Name of intermediate aperture used in NIRCam Grism time series obs.\n".format(
                    input["grismts_intermediate_aperture"]
                )
            )
            f.write(
                "  PPS_aperture: {}  # Original aperture value supplied by PPS.\n".format(
                    input["pps_aperture"]
                )
            )
            f.write(
                "  filter: {}       # Filter of simulated data (F090W, F322W2, etc)\n".format(
                    input[filtkey]
                )
            )
            f.write(
                "  pupil: {}        # Pupil element for simulated data (CLEAR, GRISMC, etc)\n".format(
                    input[pupilkey]
                )
            )
            f.write("\n")
            ...
            f.write(
                "  dark: {}   # Dark current integration used as the base\n".format(
                    input["dark"]
                )
            )
            f.write(
                "  linearized_darkfile: {}   # Linearized dark ramp to use as input. Supercedes dark above\n".format(
                    input["lindark"]
                )
            )
            f.write(
                "  badpixmask: {}   # If linearized dark is used, populate output DQ extensions using this file\n".format(
                    input["badpixmask"]
                )
            )
            f.write(
                "  superbias: {}     # Superbias file. Set to None or leave blank if not using\n".format(
                    input["superbias"]
                )
            )
            f.write(
                "  linearity: {}    # linearity correction coefficients\n".format(
                    input["linearity"]
                )
            )
            f.write(
                "  saturation: {}    # well depth reference files\n".format(
                    input["saturation"]
                )
            )
            f.write("  gain: {} # Gain map\n".format(input["gain"]))
            f.write(
                "  pixelflat: {}    # Flat field file to use for un-flattening output\n".format(
                    input["pixelflat"]
                )
            )
            f.write(
            ...
            f.write(
                "  astrometric: {}  # Astrometric distortion file (asdf)\n".format(
                    input["astrometric"]
                )
            )
            f.write(
                "  photom: {}   # cal pipeline photom reference file\n".format(
                    input["photom"]
                )
            )
            f.write(
                "  ipc: {} # File containing IPC kernel to apply\n".format(input["ipc"])
            )
            f.write(
                (
                    "  invertIPC: {}      # Invert the IPC kernel before the convolution. True or False. Use True if the kernel is "
            ...
            f.write(
                (
                    "  transmission: {}      # Transmission image containing fractional throughput map. (e.g. to imprint occulters into fov\n".format(
                        input["transmission"]
                    )
                )
            )
            f.write(
                (
                    "  subarray_defs: {} # File that contains a list of all possible subarray names and coordinates\n".format(
                        input["subarray_def_file"]
                    )
                )
            )
            f.write(
                (
                    "  readpattdefs: {}  # File that contains a list of all possible readout pattern names and associated "
                    "NFRAME/NSKIP values\n".format(input["readpatt_def_file"])
                )
            )
            f.write(
                "  crosstalk: {}   # File containing crosstalk coefficients\n".format(
                    input["crosstalk_file"]
                )
            )
            f.write(
                (
                    "  filtpupilcombo: {}   # File that lists the filter wheel element / pupil wheel element combinations. "
                    "Used only in writing output file\n".format(
                        input["filtpupilcombo_file"]
                    )
                )
            )
            f.write(
                (
                    "  filter_wheel_positions: {}  # File containing resolver wheel positions for each filter/pupil\n".format(
                        input["filter_position_file"]
                    )
                )
            )
            f.write(
                (
                    "  flux_cal: {} # File that lists flux conversion factor and pivot wavelength for each filter. Only "
            ...
            f.write(
                "  filter_throughput: {} #File containing filter throughput curve\n".format(
                    self.configfiles[instrument.lower()]["filter_throughput"]
                )
            )
            f.write("  ")
            f.write("\n")
            ...
            f.write("\n")
            f.write("cosmicRay:\n")
            cosmic_ray_path = os.path.join(
                self.datadir, instrument.lower(), "cosmic_ray_library"
            )
            f.write(
                "  path: {}               # Path to CR library\n".format(
                    cosmic_ray_path
                )
            )
            f.write(
                "  library: {}    # Type of cosmic rayenvironment (SUNMAX, SUNMIN, FLARE)\n".format(
                    input["CosmicRayLibrary"]
                )
            )
            f.write(
                "  scale: {}     # Cosmic ray scaling factor\n".format(
                    input["CosmicRayScale"]
                )
            )
            ...
            if instrument.lower() in ["nircam", "wfsc"]:
                # detector_label = input['detector']
                f.write(
                    "  suffix: IPC_NIRCam_{}    # Suffix of library file names\n".format(
                        detector_label
                    )
                )
            elif instrument.lower() == "niriss":
                f.write(
                    "  suffix: IPC_NIRISS_{}    # Suffix of library file names\n".format(
                        detector_label
                    )
                )
            elif instrument.lower() == "fgs":
                ...
                f.write(
                    "  suffix: IPC_FGS_{}    # Suffix of library file names\n".format(
                        detector_string
                    )
                )
            f.write(
                "  seed: {}                 # Seed for random number generator\n".format(
                    np.random.randint(1, 2**32 - 2)
                )
            )
            f.write("\n")
            f.write("simSignals:\n")
            ...
            f.write(
                (
                    "  pointsource: {}   #File containing a list of point sources to add (x, y locations and magnitudes)\n".format(
                        PointSourceCatalog
                    )
                )
            )
            ...
            f.write(
                "  psf_wing_threshold_file: {}   # File defining PSF sizes versus magnitude\n".format(
                    input["psf_wing_threshold_file"]
                )
            )
            f.write(
                "  add_psf_wings: {}  # Whether or not to place the core of the psf from the gridded library into an image of the wings before adding.\n".format(
                    self.add_psf_wings
                )
            )
            f.write("  psfpath: {}   #Path to PSF library\n".format(input["psfpath"]))
            f.write(
                "  psfwfe: {}   #PSF WFE value (predicted or requirements)\n".format(
                    self.psfwfe
                )
            )
            f.write(
                "  psfwfegroup: {}      #WFE realization group (0 to 4)\n".format(
                    self.psfwfegroup
                )
            )
            f.write(
                (
                    "  galaxyListFile: {}    #File containing a list of positions/ellipticities/magnitudes of galaxies "
                    "to simulate\n".format(GalaxyCatalog)
                )
            )
            f.write(
                "  extended: {}          #Extended emission count rate image file name\n".format(
                    ExtendedCatalog
                )
            )
            f.write(
                "  extendedscale: {}                          #Scaling factor for extended emission image\n".format(
                    ExtendedScale
                )
            )
            f.write(
                (
                    "  extendedCenter: {}                   #x, y pixel location at which to place the extended image "
            ...
            f.write(
                (
                    "  PSFConvolveExtended: {} #Convolve the extended image with the PSF before adding to the output "
                    "image (True or False)\n".format(self.convolve_extended)
                )
            )
            f.write(
                (
                    "  movingTargetList: {}          #Name of file containing a list of point source moving targets (e.g. "
                    "KBOs, asteroids) to add.\n".format(MovingTargetList)
                )
            )
            f.write(
                (
                    "  movingTargetSersic: {}  #ascii file containing a list of 2D sersic profiles to have moving through "
                    "the field\n".format(MovingTargetSersic)
                )
            )
            f.write(
                (
                    "  movingTargetExtended: {}      #ascii file containing a list of stamp images to add as moving targets "
                    "(planets, moons, etc)\n".format(MovingTargetExtended)
                )
            )
            f.write(
                (
                    "  movingTargetToTrack: {} #File containing a single moving target which JWST will track during "
            ...
            f.write(
                (
                    "  tso_imaging_catalog: {} #Catalog of (generally one) source for Imaging Time Series observation\n".format(
                        ImagingTSOCatalog
                    )
                )
            )
            f.write(
                (
                    "  tso_grism_catalog: {} #Catalog of (generally one) source for Grism Time Series observation\n".format(
                        GrismTSOCatalog
                    )
                )
            )
            ...
            f.write(
                (
                    "  bkgdrate: {}                         #Constant background count rate (ADU/sec/pixel in an undispersed image) or "
            ...
            f.write(
                (
                    "  poissonseed: {}                  #Random number generator seed for Poisson simulation)\n".format(
                        np.random.randint(1, 2**32 - 2)
                    )
                )
            )
            ...
            f.write(
                "  expand_catalog_for_segments: {}                     # Expand catalog for 18 segments and use distinct PSFs\n".format(
                    self.expand_catalog_for_segments
                )
            )
            f.write(
                "  use_dateobs_for_background: {}          # Use date_obs below to deternine background. If False, bkgdrate is used.\n".format(
                    self.dateobs_for_background
                )
            )
            f.write(
                "  signal_low_limit_for_segmap: {}         # Lower signal limit to include pix in segmentation map. See below for units.\n".format(
                    self.segmentation_threshold
                )
            )
            f.write(
                (
                    "  signal_low_limit_for_segmap_units: {}  # Units of signal_low_limit_for_segmap. Can be: [ADU/sec, e/sec, MJy/sr, "
                    "ergs/cm2/a, ergs/cm2/hz]\n".format(
                        self.segmentation_threshold_units
                    )
                )
            )
            f.write(
                "  add_ghosts: {}  # Add optical ghosts associated with astronomical sources\n".format(
                    self.add_ghosts
                )
            )
            f.write(
                "  PSFConvolveGhosts: {}  # Convolve ghost stamp images with instrument PSF before adding\n".format(
                    self.convolve_ghosts
                )
            )
            f.write("\n")
            f.write("Telescope:\n")
            f.write(
                "  ra: {}                      # RA of simulated pointing\n".format(
                    input["ra_ref"]
                )
            )
            f.write(
                "  dec: {}                    # Dec of simulated pointing\n".format(
                    input["dec_ref"]
                )
            )
            if "pav3" in input.keys():
                ...
                pav3_value = input["PAV3"]
            f.write(
                "  rotation: {}                    # PA_V3 in degrees, i.e. the position angle of the V3 axis at V1 (V2=0, V3=0) measured from N to E.\n".format(
                    pav3_value
                )
            )
            f.write(
                "  tracking: {}   #Telescope tracking. Can be sidereal or non-sidereal\n".format(
                    input["Tracking"]
                )
            )
            f.write("\n")
            ...
            f.write(
                (
                    "  datatype: {} # Type of data to save. 'linear' for linearized ramp. 'raw' for raw ramp. 'linear, "
            ...
            f.write(
                "  grism_source_image: {}   # grism\n".format(
                    input["grism_source_image"]
                )
            )
            f.write(
            ...
            if input["TargetRA"] != "0":
                ra_degrees, dec_degrees = utils.parse_RA_Dec(
                    input["TargetRA"], input["TargetDec"]
                )
            else:
                ra_degrees = 0.0
                dec_degrees = 0.0
            ...
            f.write(
                "  target_ra: {}  # RA of the target, from APT file.\n".format(
                    ra_degrees
                )
            )
            f.write(
                "  target_dec: {}  # Dec of the target, from APT file.\n".format(
                    dec_degrees
                )
            )
            f.write(
            ...
            f.write(
                "  observation_label: {}    # User-generated observation Label\n".format(
                    input["obs_label"].strip()
                )
            )
            f.write(
            ...
            f.write(
                "  activity_id: '{}'    # Activity ID. Increment with each exposure.\n".format(
                    input["act_id"]
                )
            )
            f.write(
                "  exposure_number: '{}'    # Exposure Number\n".format(
                    input["exposure"]
                )
            )
            f.write(
            ...
            f.write(
                "  primary_dither_type: {}  # Primary dither pattern name\n".format(
                    input["PrimaryDitherType"]
                )
            )
            f.write(
                "  total_primary_dither_positions: {}  # Total number of primary dither positions\n".format(
                    input["PrimaryDithers"]
                )
            )
            f.write(
                "  primary_dither_position: {}  # Primary dither position number\n".format(
                    int(input["primary_dither_num"])
                )
            )
            f.write(
                "  subpix_dither_type: {}  # Subpixel dither pattern name\n".format(
                    input["SubpixelDitherType"]
                )
            )
            ...
            #     val = input['SubpixelPositions']
            f.write(
                "  total_subpix_dither_positions: {}  # Total number of subpixel dither positions\n".format(
                    val
                )
            )
            f.write(
                "  subpix_dither_position: {}  # Subpixel dither position number\n".format(
                    int(input["subpix_dither_num"])
                )
            )
            f.write(
                "  xoffset: {}  # Dither pointing offset in x (arcsec)\n".format(
                    input["idlx"]
                )
            )
            f.write(
                "  yoffset: {}  # Dither pointing offset in y (arcsec)\n".format(
                    input["idly"]
                )
            )
    ...
    def add_options(self, parser=None, usage=None):
        if parser is None:
            parser = argparse.ArgumentParser(
                usage=usage, description="Simulate JWST ramp"
            )
        parser.add_argument(
            "--input_xml", help="XML file from APT describing the observations."
        )
        parser.add_argument(
            "--pointing_file", help="Pointing file from APT describing observations."
        )
        parser.add_argument(
            "--datatype",
            help='Type of data to save. Can be "linear", "raw" or "linear, raw"',
            default="linear",
        )
        parser.add_argument(
            "--output_dir",
            help="Directory into which the yaml files are output",
            default="./",
        )
        parser.add_argument(
            "--table_file",
            help="Ascii table containing observation info. Use this or xml + pointing files.",
            default=None,
        )
        parser.add_argument(
            "--use_nonstsci_names",
            help="Use STScI naming convention for output files",
            action="store_true",
        )
        parser.add_argument(
            "--subarray_def_file",
            help="Ascii file containing subarray definitions",
            default="config",
        )
        parser.add_argument(
            "--readpatt_def_file",
            help="Ascii file containing readout pattern definitions",
            default="config",
        )
        parser.add_argument(
            "--crosstalk", help="Crosstalk coefficient file", default="config"
        )
        parser.add_argument(
            "--filtpupil_pairs",
            help="List of paired filter/pupil elements",
            default="config",
        )
        parser.add_argument(
            "--fluxcal", help="File with zeropoints per filter", default="config"
        )
        parser.add_argument(
            "--dq_init_config", help="DQ Initialization config file", default="config"
        )
        parser.add_argument(
            "--saturation_config", help="Saturation config file", default="config"
        )
        parser.add_argument(
            "--superbias_config",
            help="Superbias subtraction config file",
            default="config",
        )
        parser.add_argument(
            "--refpix_config", help="Refpix subtraction config file", default="config"
        )
        parser.add_argument(
            "--linearity_config", help="Linearity config file", default="config"
        )
        parser.add_argument(
            "--observation_list_file",
            help="Table file containing epoch start times, telescope roll angles, catalogs for each observation",
            default=None,
        )
        parser.add_argument(
            "--use_JWST_pipeline", help="True/False", action="store_true"
        )
        parser.add_argument(
            "--use_linearized_darks", help="True/False", action="store_true"
        )
        parser.add_argument(
            "--simdata_output_dir",
            help="Output directory for simulated exposure files",
            default="./",
        )
        parser.add_argument(
            "--psfpath",
            help="Directory containing PSF library",
            default=os.path.join(self.datadir, "gridded_psf_library"),
        )
        parser.add_argument(
            "--psfbasename",
            help="Basename of the files in the PSF library",
            default="nircam",
        )
        parser.add_argument(
            "--psfpixfrac",
            help="Subpixel centering resolution of files in PSF library",
            default=0.25,
        )
        parser.add_argument(
            "--psfwfe",
            help="Wavefront error value to use for PSFs",
            default="predicted",
        )
        parser.add_argument(
            "--psfwfegroup", help="Realization index number for PSF files", default=0
        )
        parser.add_argument(
            "--resets_bet_ints",
            help="Number of detector resets between integrations",
            default=1,
        )
        parser.add_argument(
            "--tracking",
            help="Type of telescope tracking: 'sidereal' or 'non-sidereal'",
            default="sidereal",
        )
...
def _gtvt_v3pa_on_date(ra, dec, date=None, return_range=False):
    ...
    tbl = jwst_gtvt.find_tgt_info.get_table(
        ra=ra,
        dec=dec,
        instrument="NIRCam",
...
def default_obs_v3pa_on_date(
    pointing_filename, obs_num, date=None, verbose=False, pointing_table=None
):
    ...
    for i in range(len(pointing_table["obs_num"])):
        if pointing_table["obs_num"][i] == f"{obs_num:03d}":
            ...
            if verbose:
                ...
                self.logger.info(
                    f" Coords from APT pointing file: {ra_deg} {dec_deg} deg"
                )
            break
    else:
        raise RuntimeError(
            f"Could not find any info for an observation number {obs_num} in the pointing table."
        )

    result = _gtvt_v3pa_on_date(ra_deg, dec_deg, date=date)
    if np.isnan(result):
        raise RuntimeError(
            "Obs {obs_num} is not observable on date {date}. Target not in the field of regard."
        )
    return result
...
def all_obs_v3pa_on_date(pointing_filename, date=None, verbose=False):
    ...
    for obs_num in obsnums:
        results[obs_num] = default_obs_v3pa_on_date(
            pointing_filename,
            int(obs_num),
            date=date,
            verbose=verbose,
