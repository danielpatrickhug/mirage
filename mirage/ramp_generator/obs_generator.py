...
from mirage.utils.constants import (
    ...
    NUM_RESETS_BEFORE_EXP,
    NUM_RESETS_BEFORE_INT,
    TABLE_BASETIME,
)
...
class Observation:
    def __init__(self, offline=False):
        ...
        self.coord_adjust = {"x": 1.0, "xoffset": 0.0, "y": 1.0, "yoffset": 0.0}
    ...
    def add_crosstalk(self, exposure):
        ...
        if self.params["Readout"]["namp"] == 4:
            if self.instrument.upper() == "NIRCAM":
                ...
                xdet = self.detector
            xtcoeffs = self.read_crosstalk_file(
                self.params["Reffiles"]["crosstalk"], xdet
            )
            ...
            self.logger.info(
                (
                    "Crosstalk calculation requested, but the chosen subarray "
        ...
        if num_integrations is None:
            ...
            # TSO and moving targets
                ...
                if num_integrations != seed.shape[0]:
                    raise ValueError(
                        (
                            "The number of integrations reported for the dark ({}) does not match that "
    ...
    def add_flatfield_effects(self, ramp):
        ...
        if self.runStep["illuminationflat"]:
            illuminationflat, illuminationflatheader = self.read_cal_file(
        ...
        if self.runStep["pixelflat"]:
            pixelflat, pixelflatheader = self.read_cal_file(
        ...
        for integration in range(shape[0]):
            for group in range(shape[1]):
                ...
                temp[b_b : b_b + ny, l_b : l_b + nx] = output_data[
                    integration, group, yoff : yoff + ny, xoff : xoff + nx
                ].copy()
                ...
                if len(kshape) == 2:
                    ...
                    k_temp = np.zeros(
                        (kshape[0], kshape[1], tny, tnx), dtype=kernel.dtype
                    )
    ...
    def add_superbias_and_refpix(self, ramp, sbref):
        ...
        if len(rampdim) != len(sbrefdim):
            if len(rampdim) == (len(sbrefdim) + 1):
                ...
                raise ValueError(
                    (
                        "WARNING: input ramp and superbias+refpix "
                        "arrays have different dimensions. Cannot combine. "
                        "Ramp dim: {}, SBRef dim: {}".format(
                            len(rampdim), len(sbrefdim)
                        )
                    )
                )
        else:
    ...
    def add_synthetic_to_dark(self, synthetic, dark, syn_zeroframe=None):
        ...
        if (darkpatt in rapids) and (self.params["Readout"]["readpatt"] not in rapids):
            deltaframe = (
                self.params["Readout"]["nskip"] + self.params["Readout"]["nframe"]
            )
            ...
            for integ in range(self.params["Readout"]["nint"]):
                ...
                for i in range(self.params["Readout"]["ngroup"]):
                    ...
                    self.logger.info(
                        (
                            "Averaging dark current ramp in add_synthetic_to_dark."
                    ...
                    if self.params["Readout"]["nframe"] > 1:
                        ...
                        sbaccumimage = np.mean(
                            dark.sbAndRefpix[integ, frames, :, :], axis=0
                        )
            ...
            synthetic = (
                synthetic + dark.data[:, 0 : self.params["Readout"]["ngroup"], :, :]
            )
    ...
    def check_params(self):
        ...
        if self.params["Inst"]["instrument"].lower() not in INST_LIST:
            raise ValueError(
                (
                    "WARNING: instrument {} not in the list of "
                    "available instruments: {}".format(
                        self.params["Inst"]["instrument"].lower(), INST_LIST
                    )
                )
            )
        ...
        if self.params["Inst"]["mode"] in possibleModes:
            ...
            raise ValueError(
                (
                    "WARNING: unrecognized mode {}. Must be one of: {}".format(
                        self.params["Inst"]["mode"], possibleModes
                    )
                )
            )
        ...
        self.runStep = {}
        self.runStep["superbias"] = self.check_run_step(
            self.params["Reffiles"]["superbias"]
        )
        self.runStep["nonlin"] = self.check_run_step(
            self.params["Reffiles"]["linearity"]
        )
        ...
        self.runStep["pixelflat"] = self.check_run_step(
            self.params["Reffiles"]["pixelflat"]
        )
        self.runStep["illuminationflat"] = self.check_run_step(
            self.params["Reffiles"]["illumflat"]
        )
        self.runStep["astrometric"] = self.check_run_step(
            self.params["Reffiles"]["astrometric"]
        )
        self.runStep["ipc"] = self.check_run_step(self.params["Reffiles"]["ipc"])
        self.runStep["crosstalk"] = self.check_run_step(
            self.params["Reffiles"]["crosstalk"]
        )
        self.runStep["occult"] = self.check_run_step(self.params["Reffiles"]["occult"])
        self.runStep["pointsource"] = self.check_run_step(
            self.params["simSignals"]["pointsource"]
        )
        self.runStep["galaxies"] = self.check_run_step(
            self.params["simSignals"]["galaxyListFile"]
        )
        self.runStep["extendedsource"] = self.check_run_step(
            self.params["simSignals"]["extended"]
        )
        self.runStep["movingTargets"] = self.check_run_step(
            self.params["simSignals"]["movingTargetList"]
        )
        self.runStep["movingTargetsSersic"] = self.check_run_step(
            self.params["simSignals"]["movingTargetSersic"]
        )
        self.runStep["movingTargetsExtended"] = self.check_run_step(
            self.params["simSignals"]["movingTargetExtended"]
        )
        self.runStep["MT_tracking"] = self.check_run_step(
            self.params["simSignals"]["movingTargetToTrack"]
        )
        self.runStep["zodiacal"] = self.check_run_step(
            self.params["simSignals"]["zodiacal"]
        )
        self.runStep["scattered"] = self.check_run_step(
            self.params["simSignals"]["scattered"]
        )
        self.runStep["linearity"] = self.check_run_step(
            self.params["Reffiles"]["linearity"]
        )
        self.runStep["cosmicray"] = self.check_run_step(
            self.params["cosmicRay"]["path"]
        )
        self.runStep["saturation_lin_limit"] = self.check_run_step(
            self.params["Reffiles"]["saturation"]
        )
        self.runStep["fwpw"] = self.check_run_step(
            self.params["Reffiles"]["filtpupilcombo"]
        )
        self.runStep["linearized_darkfile"] = self.check_run_step(
            self.params["Reffiles"]["linearized_darkfile"]
        )
        self.runStep["badpixfile"] = self.check_run_step(
            self.params["Reffiles"]["badpixmask"]
        )

        # NON-LINEARITY
        # Make sure the input accuracy is a float with reasonable bounds
        self.params["nonlin"]["accuracy"] = self.check_param_val(
            self.params["nonlin"]["accuracy"], "nlin accuracy", 1e-12, 1e-6, 1e-6
        )
        self.params["nonlin"]["maxiter"] = self.check_param_val(
            self.params["nonlin"]["maxiter"], "nonlin max iterations", 5, 40, 10
        )
        self.params["nonlin"]["limit"] = self.check_param_val(
            self.params["nonlin"]["limit"], "nonlin max value", 30000.0, 1.0e6, 66000.0
        )
        ...
        try:
            ...
            self.logger.warning(
                (
                    "ERROR: cosmic ray random number generator seed is bad. "
                    "Using the default value of {}.".format(
                        self.params["cosmicRay"]["seed"]
                    )
                )
            )
        ...
        try:
            self.params["simSignals"]["poissonseed"] = int(
                self.params["simSignals"]["poissonseed"]
            )
        except:
        ...
        if self.params["cosmicRay"]["path"] is None:
            ...
            if self.params["cosmicRay"]["path"][-1] != "/":
                self.params["cosmicRay"]["path"] += "/"
            if self.params["cosmicRay"]["library"].upper() in [
                "SUNMAX",
                "SUNMIN",
                "FLARES",
            ]:
                self.crfile = os.path.join(
                    self.params["cosmicRay"]["path"],
                ...
                raise FileNotFoundError(
                    (
                        "Warning: unrecognised cosmic ray library {}".format(
                            self.params["cosmicRay"]["library"]
                        )
                    )
                )
        ...
        self.params["cosmicRay"]["scale"] = self.check_param_val(
            self.params["cosmicRay"]["scale"], "cosmicRay", 0, 100, 1
        )
        self.params["simSignals"]["extendedscale"] = self.check_param_val(
            self.params["simSignals"]["extendedscale"], "extendedEmission", 0, 10000, 1
        )
        self.params["simSignals"]["zodiscale"] = self.check_param_val(
            self.params["simSignals"]["zodiscale"], "zodi", 0, 10000, 1
        )
        self.params["simSignals"]["scatteredscale"] = self.check_param_val(
        ...
        if self.instrument.upper() in ["NIRCAM", "NIRISS"]:
            ...
            if self.instrument.upper() == "NIRISS":
                ...
                if "5" in self.detector or "LONG" in self.detector.upper():
                    ...
                    channel = "SW"
                f_match = (self.params["Readout"]["filter"] == fw_positions["Name"]) & (
                    channel == fw_positions["Channel"]
                )
                p_match = (self.params["Readout"]["pupil"] == fw_positions["Name"]) & (
                    channel == fw_positions["Channel"]
                )
            self.filter_wheel_position = fw_positions[
                "Filter_Resolver_Reading_Wheel_Degrees"
            ][f_match].data[0]
            self.pupil_wheel_position = fw_positions[
                "Pupil_Resolver_Reading_Wheel_Degrees"
            ][p_match].data[0]
        elif self.instrument.upper() == "FGS":
            self.filter_wheel_position = 999.0
            self.pupil_wheel_position = 999.0
    ...
    def combine_seeds(self, filenames):
        ...
        for i, filename in enumerate(filenames):
            ...
            if i == 0:
                ...
                self.logger.info(
                    (
                        "Final seed image shape before averaging/skipping frames to create groups: "
            ...
            seed[
                int_start : int_start + ints, grp_start : grp_start + groups, :, :
            ] = seed_data
        ...
        if (
            ...
            for record in dq_table:
                ...
                try:
                    ...
                    self.logger.info(
                        (
                            "Keyword {} does not correspond to an existing DQ "
    ...
    def cr_funcs(self, npix, seed=4242):
        ...
        # be consistent.
        crs_perframe = np.random.poisson(
            crhits,
    ...
    def create(self, override_refs=None):
        ...
        self.rampexptime = self.frametime * (
            self.params["Readout"]["ngroup"]
            * (self.params["Readout"]["nframe"] + self.params["Readout"]["nskip"])
        )
        ...
        if self.params["Reffiles"]["saturation"] is not None:
            ...
            self.logger.warning(
                (
                    "No saturation map provided. Using "
        ...
        if len(self.linDark) > 1:
            self.logger.info(
                (
                    "An estimate of the remaining processing time will be provided after the first "
        ...
        for i, linDark in enumerate(self.linDark):
            ...
            if seg_str != "":
                ...
                try:
                    ...
                    if len(parts) == 5:
                        basename = "{}_{}_{}-seg{}_{}_{}".format(
                            parts[0], parts[1], parts[2], seg_str, parts[3], parts[4]
                        )
                    else:
                        basename = basename.replace(
                            ".fits", "-seg{}.fits".format(seg_str)
                        )
                except IndexError:
            ...
            if isinstance(seed_files, str):
                ...
                self.seed_image, self.segmap, self.seedheader = self.read_seed(
                    seed_files
                )
            elif isinstance(seed_files, list):
                # If the seed image is a list of files (due to high data
                # volume)
                self.seed_image, self.segmap, self.seedheader = self.combine_seeds(
                    seed_files
                )
            else:
            ...
            if "UNITS" in self.seedheader.keys():
                if self.seedheader["UNITS"] in ["e-/sec", "e-"]:
                    self.logger.info(
                        (
                            "Seed image is in units of {}. Dividing by gain.".format(
                                self.seedheader["units"]
                            )
                        )
                    )
                ...
                raise ValueError(
                    (
                        "'UNITS' keyword not present in header of "
            ...
            simexp, simzero = self.add_crs_and_noise(
                self.seed_image, num_integrations=num_integrations
            )

            # Multiply flat fields
            simexp = self.add_flatfield_effects(simexp)
            simzero = self.add_flatfield_effects(np.expand_dims(simzero, axis=1))[
                :, 0, :, :
            ]
            ...
            lin_outramp, lin_zeroframe, lin_sbAndRefpix = self.add_synthetic_to_dark(
                simexp, self.linear_dark, syn_zeroframe=simzero
            )
            ...
            lin_zeroframe = self.add_detector_effects(
                np.expand_dims(lin_zeroframe, axis=1)
            )[:, 0, :, :]
            ...
            limits = np.zeros_like(self.satmap) + 1.0e6

            if self.linear_dark.sbAndRefpix is not None:
                lin_satmap = unlinearize.nonLinFunc(
                    self.satmap - self.linear_dark.sbAndRefpix[0, 0, :, :],
                    nonlincoeffs,
                    limits,
                )
                ...
                lin_satmap = unlinearize.nonLinFunc(
                    self.satmap - self.superbias, nonlincoeffs, limits
                )

            elif (self.linear_dark.sbAndRefpix is None) & (
                self.runStep["superbias"] is False
            ):
                ...
                # pixels.
                manual_sb = np.zeros_like(self.satmap) + 12000.0
                lin_satmap = unlinearize.nonLinFunc(
                    self.satmap - manual_sb, nonlincoeffs, limits
                )
            ...
            if "linear" in self.params["Output"]["datatype"].lower():
                ...
                linearrampfile = os.path.join(
                    self.params["Output"]["directory"], linearrampfile
                )
            ...
            if "raw" in self.params["Output"]["datatype"].lower():
                if self.linear_dark.sbAndRefpix is not None:
                    ...
                    raw_outramp = unlinearize.unlinearize(
                        lin_outramp,
                        nonlincoeffs,
                        self.satmap,
                        lin_satmap,
                    ...
                    raw_zeroframe = unlinearize.unlinearize(
                        lin_zeroframe,
                        nonlincoeffs,
                        self.satmap,
                        lin_satmap,
                    ...
                    raw_outramp = self.add_superbias_and_refpix(
                        raw_outramp, lin_sbAndRefpix
                    )
                    raw_zeroframe = self.add_superbias_and_refpix(
                        raw_zeroframe, self.linear_dark.zero_sbAndRefpix
                    )
                    ...
                    # Save the raw ramp
                    # base_name = self.params['Output']['file'].split('/')[-1]
                    rawrampfile = os.path.join(
                        self.params["Output"]["directory"], basename
                    )
                    if self.params["Inst"]["use_JWST_pipeline"]:
                        self.save_DMS(raw_outramp, raw_zeroframe, rawrampfile, mod="1b")
                    else:
                        self.save_fits(
                            raw_outramp, raw_zeroframe, rawrampfile, mod="1b"
                        )
                    ...
                    raise ValueError(
                        (
                            "WARNING: raw output ramp requested, but the signal associated "
            ...
            if len(self.linDark) > 1:
                ...
                estimated_remaining_time = (
                    time_per_segment * (len(self.linDark) - (i + 1)) * u.second
                )
                ...
                self.logger.info(
                    (
                        "Estimated time remaining in obs_generator: {} minutes. "
    ...
    def crosstalk_image(self, orig, coeffs):
        ...
        if self.params["Output"]["save_intermediates"] is True:
            ...
            base_name = self.params["Output"]["file"].split("/")[-1]
            xtalkout = os.path.join(
                self.params["Output"]["directory"],
                base_name[0:-5] + "_xtalk_correction_image.fits",
            )
    ...
    def dark_setup(self):
        ...
        if isinstance(self.linDark, mirage.utils.read_fits.Read_fits):
            ...
            if self.linDark is None:
                ...
                raise TypeError(
                    "Unsupported type for self.linDark: {}".format(type(self.linDark))
                )
    ...
    def do_cosmic_rays(self, image, ngroup, iframe, ncr, seedval):
        ...
        while i < nray:
            ...
            image[i1:i2, j1:j2] = (
                image[i1:i2, j1:j2] + crimage[k1:k2, l1:l2] / self.gain
            )  # self.gainim[k1:k2, l1:l2]
    ...
    def frame_to_ramp(self, data):
        ...
        if self.runStep["cosmicray"]:
            ...
            crhits, crs_perframe = self.cr_funcs(
                npix, seed=self.params["cosmicRay"]["seed"]
            )

            # open output file to contain the list of cosmic rays
            base_name = self.params["Output"]["file"].split("/")[-1]
            crlistout = os.path.join(
                self.params["Output"]["directory"], base_name[0:-5] + "_cosmicrays.list"
            )
        ...
        for i in range(self.params["Readout"]["ngroup"]):
            ...
            for j in range(rstart, framesPerGroup):
                ...
                # Add poisson noise
                poissonsignal = self.do_poisson(
                    deltaframe, self.params["simSignals"]["poissonseed"]
                )
                ...
                # Add cosmic rays
                if self.runStep["cosmicray"]:
                    framesignal = self.do_cosmic_rays(
                        framesignal,
                        i,
                        j,
                ...
                if j >= self.params["Readout"]["nskip"]:
                    self.logger.info(
                        "    Averaging frame {} into group {}".format(frameindex, i)
                    )
    ...
    def frame_to_ramp_no_cr(self, data):
        ...
        for i in range(self.params["Readout"]["ngroup"]):
            ...
            for j in range(framesPerGroup):
                ...
                # Add poisson noise
                if ndim == 3:
                    framesignal = self.do_poisson(
                        data[frameindex + 1], self.params["simSignals"]["poissonseed"]
                    )
                elif ndim == 2:
                    framesignal = self.do_poisson(
                        data * frameindex, self.params["simSignals"]["poissonseed"]
                    )
                ...
                # Add the frame to the group signal image
                if (self.params["Readout"]["nskip"] > 0) & (
                    j >= self.params["Readout"]["nskip"]
                ):
                    self.logger.info(
                        "    Averaging frame {} into group {}".format(frameindex, i)
                    )
                    accumimage += framesignal
                elif (self.params["Readout"]["nskip"] > 0) & (
                    j < self.params["Readout"]["nskip"]
                ):
                    self.logger.info("    Skipping frame {}".format(frameindex))
    ...
    def get_cr_rate(self):
        ...
        if self.crrate > 0.0:
            self.logger.info(
                "Base cosmic ray probability per pixel per second: {}".format(
                    self.crrate
                )
            )
    ...
    def get_nonlin_coeffs(self, linfile):
        ...
        if numnan > 0:
            self.logger.info(
                (
                    "The linearity coefficients of {} pixels are NaNs. "
    ...
    def get_nonlinearity_coeffs(self):
        ...
        if self.params["Reffiles"]["linearity"] is not None:
            try:
                ...
                self.logger.warning(
                    (
                        "Unable to read in non-linearity correction coefficients "
            ...
            self.logger.info(
                (
                    "No linearity coefficient file provided. Proceeding using a "
    ...
    def invert_ipc_kernel(self, kern):
        ...
        self.logger.info(
            (
                "Inverted IPC kernel saved to {} for future simulator "
                "runs.".format(outname)
            )
        )
        return newkernel
    ...
    def mask_refpix(self, ramp, zero):
        ...
        maskimage[4 : self.ffsize - 4, 4 : self.ffsize - 4] = 1.0
    ...
    def open_cr_list_file(self, filename, hits):
        ...
        self.cosmicraylist.write(
            "# Cosmic ray list (file set %s random seed %d)\n"
            % (self.crfile, self.params["cosmicRay"]["seed"])
        )
        self.cosmicraylist.write(
            "# Cosmic ray rate per frame: %13.6e (scale factor %f)\n"
            % (hits, self.params["cosmicRay"]["scale"])
        )
        self.cosmicraylist.write(
            (
                "Image_x    Image_y    Group   Frame   CR_File_Index   CR_file_frame   "
                "Max_CR_Signal\n"
            )
        )
    ...
    def populate_group_table(
        self, starttime, grouptime, ramptime, numint, numgroup, ny, nx
    ):
        ...
        grouptable = self.create_group_entry(
            999, 999, 0, 0, 0, "void", 0, 0, 0, 0, "void", 1.0, 1.0
        )
        ...
        for integ in range(numint):
            ...
            for grp, day, milli, submilli, grpstr, bary, helio in zip(
                groups,
                enddayint,
                endmilli_int,
                endsubmilli,
    ...
    def read_cal_file(self, filename):
        ...
        if (
            (self.subarray_bounds[0] != 0)
            or (self.subarray_bounds[2] != (self.ffsize - 1))
            or (self.subarray_bounds[1] != 0)
            or (self.subarray_bounds[3] != (self.ffsize - 1))
        ):
    ...
    def read_crosstalk_file(self, file, detector):
        ...
        if np.any(mtch) is False:
            raise ValueError(
                "Detector {} not found in xtalk file {}".format(detector, file)
            )
    ...
    def read_gain_map(self):
        ...
        if self.runStep["gain"]:
            self.gainim, self.gainhead = self.read_cal_file(
                self.params["Reffiles"]["gain"]
            )
    ...
    def read_parameter_file(self):
        ...
        if self.params["Inst"]["instrument"].lower() == "niriss":
            newfilter, newpupil = utils.check_niriss_filter(
    ...
    def read_saturation_file(self):
        ...
        if self.runStep["saturation_lin_limit"]:
            try:
                self.satmap, self.satheader = self.read_cal_file(
                    self.params["Reffiles"]["saturation"]
                )
                bad = ~np.isfinite(self.satmap)
                self.satmap[bad] = 1.0e6
            except Exception:
                self.logger.warning(
                    (
                        "WARNING: unable to open saturation file {}.".format(
                            self.params["Reffiles"]["saturation"]
                        )
                    )
                )
                self.logger.warning(
                    (
                        "Please provide a valid file, or place 'none' "
                        "in the saturation entry in the parameter file, "
                        "in which case the nonlin limit value in the "
                        "parameter file ({}) will be used for all pixels.".format(
    ...
    def read_superbias_file(self):
        ...
        if self.runStep["superbias"]:
            try:
                self.superbias, self.superbiasheader = self.read_cal_file(
                    self.params["Reffiles"]["superbias"]
                )
            except:
                raise IOError(
                    (
                        "WARNING: unable to open superbias file {}. "
                        "Please provide a valid file in the superbias "
                        "entry in the parameter file.".format(
                            self.params["Reffiles"]["superbias"]
                        )
                    )
                )
    ...
    def readpattern_compatible(self):
        ...
        if (darkpatt != self.params["Readout"]["readpatt"]) & (darkpatt not in rapids):
            raise ValueError(
                (
                    "WARNING: Unable to convert input dark with a "
                    ...
                    "readout pattern.".format(
                        darkpatt, self.params["Readout"]["readpatt"]
                    )
                )
            )
    ...
    def int_times_table(self, integration_time, date_obs, time_obs, num_ints):
        ...
        integ_0_time_delta = TimeDelta(
            (integration_time - self.num_resets_before_exposure * self.frametime)
            * u.second
        )
        ...
        start_times = (
            start_time
            + integ_0_time_delta
            + (integ_time_delta * (integration_numbers - 1))
        )

        integration_time_exclude_reset = TimeDelta(
            (integration_time - self.frametime) * u.second
        )
        end_times = start_times + integration_time_exclude_reset

        mid_times = start_times + integration_time_exclude_reset / 2.0
        ...
        data_list = [
            (
                integration_numbers[i] + 1,
                start_times.mjd[i],
                mid_times.mjd[i],
                end_times.mjd[i],
                start_times_bjd.mjd[i],
                mid_times_bjd.mjd[i],
                end_times_bjd.mjd[i],
            )
    ...
    def save_DMS(
        ...
        if mod == "1b":
            ...
            raise ValueError(
                (
                    "Model type to use for saving output is "
        ...
        try:
            outModel.meta.exposure.type = EXPTYPES[
        ...
        with warnings.catch_warnings():
            ...
            start_time_string = (
                str(self.params["Output"]["date_obs"])
                + " "
                + str(self.params["Output"]["time_obs"])
            )
        ...
        if self.instrument.upper() in ["NIRISS", "FGS"]:
            outModel.meta.instrument.focus_position = (
                0.0  # Placeholder, required by WSS; will be float in flight.
            )
        ...
        outModel.meta.observation.program_number = self.params["Output"][
            "program_number"
        ]
        outModel.meta.observation.observation_number = self.params["Output"][
            "observation_number"
        ]
        outModel.meta.observation.observation_label = self.params["Output"][
            "observation_label"
        ]
        ...
        outModel.meta.observation.exposure_number = self.params["Output"][
            "exposure_number"
        ]

        outModel.meta.program.pi_name = utils.ensure_ascii(
            self.params["Output"]["PI_Name"]
        )
        ...
        outModel.meta.program.science_category = self.params["Output"][
            "Science_category"
        ]
        ...
        outModel.meta.wcsinfo.cdelt1 = self.siaf.XSciScale / 3600.0
        outModel.meta.wcsinfo.cdelt2 = self.siaf.YSciScale / 3600.0
        ...
        try:
            ...
            self.logger.warning(
                "Unable to propagate XREF_SCI, YREF_SCI from seed image to simulated data file."
            )

        # ra_v1, dec_v1, and pa_v3 are not used by the level 2 pipelines
        # compute pointing of V1 axis
        pointing_ra_v1, pointing_dec_v1 = pysiaf.rotations.pointing(
            self.attitude_matrix, 0.0, 0.0
        )
        ...
        int_times = self.int_times_table(
            self.ramptime,
            self.params["Output"]["date_obs"],
            self.params["Output"]["time_obs"],
        ...
        if isinstance(self.params["Output"]["total_primary_dither_positions"], str):
            num_primary_dithers = int(
                self.params["Output"]["total_primary_dither_positions"][0]
            )

        outModel.meta.dither.primary_type = self.params["Output"][
            "primary_dither_type"
        ].upper()
        outModel.meta.dither.position_number = self.params["Output"][
            "primary_dither_position"
        ]
        outModel.meta.dither.total_points = num_primary_dithers
        outModel.meta.dither.dither_points = int(
            self.params["Output"]["total_primary_dither_positions"]
        )
        outModel.meta.dither.pattern_size = "DEFAULT"
        outModel.meta.dither.subpixel_type = self.params["Output"]["subpix_dither_type"]
        outModel.meta.dither.subpixel_number = self.params["Output"][
            "subpix_dither_position"
        ]
        outModel.meta.dither.subpixel_total_points = self.params["Output"][
        ...
        xc = (self.subarray_bounds[2] + self.subarray_bounds[0]) / 2.0 + 1.0
        yc = (self.subarray_bounds[3] + self.subarray_bounds[1]) / 2.0 + 1.0
        ...
        if not any(mtch):
            if (
                "_MASKLWB" in self.params["Readout"]["array_name"]
                or "_MASKSWB" in self.params["Readout"]["array_name"]
            ):
                ...
                if not any(mtch):
                    raise ValueError(
                        "Unrecognized aperture name: {}".format(
                            self.params["Readout"]["array_name"]
                        )
                    )
        ...
        outModel.meta.subarray.xsize = (
            self.subarray_bounds[2] - self.subarray_bounds[0] + 1
        )
        outModel.meta.subarray.ysize = (
        ...
        outModel.meta.exposure.group_time = self.frametime * (
            self.params["Readout"]["nframe"] + self.params["Readout"]["nskip"]
        )
        ...
        outModel.meta.exposure.exposure_time = (
            self.rampexptime * self.params["Readout"]["nint"]
        )
        outModel.meta.model_type = "RampModel"

        # set the exposure start time
        outModel.meta.exposure.start_time = ct.mjd
        endingTime = ct.mjd + outModel.meta.exposure.exposure_time / 3600.0 / 24.0
        outModel.meta.exposure.end_time = endingTime
        outModel.meta.exposure.mid_time = (
            ct.mjd + outModel.meta.exposure.exposure_time / 3600.0 / 24.0 / 2.0
        )
        ...
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            outModel.group = self.populate_group_table(
                ct,
                outModel.meta.exposure.group_time,
                self.rampexptime,
                n_int,
                n_group,
                n_y,
        ...
        if self.params["Telescope"]["tracking"] == "non-sidereal":
            ...
            nonsidereal_cat, pixFlag, velFlag, magsys = file_io.readMTFile(
                self.params["simSignals"]["movingTargetToTrack"]
            )
            if nonsidereal_cat["ephemeris_file"] != "None":
                ...
                # datetime object.
                ra_interp_function, dec_interp_function = ephemeris_tools.get_ephemeris(
                    nonsidereal_cat["ephemeris_file"].data[0]
                )
            else:
                # No ephemeris file
                raise ValueError(
                    "Moving target table with no ephemeris not yet supported."
                )

            # We need to populate the MT_RA and MT_DEC keywords, which list the target RA and Dec at the
            # mid-time of the exposure.
            mid_time_datetime = moving_target_position_table.obstime_to_datetime(
                outModel.meta.exposure.mid_time
            )
            ...
            starttime_datetime = moving_target_position_table.obstime_to_datetime(
            ...
            mt_v2, mt_v3 = pysiaf.utils.rotations.getv2v3(
                self.attitude_matrix, mt_start_ra, mt_start_dec
            )
            mt_x, mt_y = self.siaf.tel_to_sci(mt_v2, mt_v3)
            outModel.moving_target = (
                moving_target_position_table.populate_moving_target_table(
                    outModel.group,
                    ephem_interp_function,
                    mt_x,
                    mt_y,
    ...
    def save_fits(
        ...
        try:
            outModel[0].header["EXP_TYPE"] = EXPTYPES[
                self.params["Inst"]["instrument"].lower()
            ][self.params["Inst"]["mode"].lower()]
        except:
            raise ValueError("EXPTYPE mapping not complete for this!!! FIX ME!")
        ...
        outModel[0].header["PI_NAME"] = utils.ensure_ascii(
            self.params["Output"]["PI_Name"]
        )
        ...
        outModel[1].header["CDELT1"] = self.siaf.XSciScale / 3600.0
        outModel[1].header["CDELT2"] = self.siaf.YSciScale / 3600.0
        ...
        if self.instrument.upper() in ["NIRISS", "FGS"]:
            outModel[0].header[
                "FOCUSPOS"
            ] = "DEFAULT"  # Placeholder, required by WSS; will be float in flight.
        ...
        num_primary_dithers = self.params["Output"]["total_primary_dither_positions"]
        if isinstance(self.params["Output"]["total_primary_dither_positions"], str):
            num_primary_dithers = np.int(
                self.params["Output"]["total_primary_dither_positions"][0]
            )

        outModel[0].header["PATTTYPE"] = self.params["Output"]["primary_dither_type"]
        outModel[0].header["PATT_NUM"] = self.params["Output"][
            "primary_dither_position"
        ]
        outModel[0].header["NUMDTHPT"] = num_primary_dithers
        outModel[0].header["NDITHPTS"] = str(
            self.params["Output"]["total_primary_dither_positions"]
        )
        outModel[0].header["NRIMDTPT"] = int(
            self.params["Output"]["total_primary_dither_positions"]
        )
        ...
        outModel[0].header["SUBPXPNS"] = self.params["Output"][
            "total_subpix_dither_positions"
        ]
        ...
        outModel[0].header["SUBSIZE1"] = (
            self.subarray_bounds[2] - self.subarray_bounds[0] + 1
        )
        outModel[0].header["SUBSIZE2"] = (
            self.subarray_bounds[3] - self.subarray_bounds[1] + 1
        )
        ...
        outModel[0].header["TGROUP"] = self.frametime * (
            self.params["Readout"]["nframe"] + self.params["Readout"]["nskip"]
        )
        ...
        outModel[0].header["EFFEXPTM"] = (
            self.rampexptime * self.params["Readout"]["nint"]
        )

        # set the exposure start time as the current time
        outModel[0].header["EXPSTART"] = ct.mjd
        outModel[0].header["EXPEND"] = (
            ct.mjd + outModel[0].header["EFFEXPTM"] / 3600.0 / 24.0
        )
        outModel[0].header["EXPMID"] = (
            ct.mjd + outModel[0].header["EFFEXPTM"] / 3600.0 / 24.0 / 2.0
        )
        ...
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            outModel[groupextnum].data = self.populate_group_table(
                ct,
                outModel[0].header["TGROUP"],
                self.rampexptime,
                n_int,
                n_group,
                n_y,
    ...
    def resets_before_exp(self):
        ...
        self.num_resets_before_exposure = NUM_RESETS_BEFORE_EXP[
    ...
    def get_duration(self):
        ...
        """
        total_photon_collection_time = self.frametime * (
            (
                self.params["Readout"]["ngroup"] * self.params["Readout"]["nframe"]
                + (self.params["Readout"]["ngroup"] - 1)
                * self.params["Readout"]["nskip"]
            )
            * self.params["Readout"]["nint"]
        )
        duration = total_photon_collection_time + self.frametime * (
            self.num_resets_before_exposure
            + NUM_RESETS_BEFORE_INT[self.instrument.lower()]
            * (self.params["Readout"]["nint"] - 1)
        )
        ...
        if (
            ...
            and "CEN" not in self.params["Readout"]["array_name"]
        ):
    ...
    def add_options(self, parser=None, usage=None):
        ...
        parser.add_argument(
            "seed", help="File containing seed image and segmentation map"
        )
        return parser
...
if __name__ == "__main__":
    ...
    usagestring = "USAGE: obs_generator.py inputs.yaml " "lindark.fits seedimg.fits"
