...
from mirage.utils.constants import (
    ...
    PSF_NORM_MIN,
    PSF_NORM_MAX,
    NIRISS_NRM_PSF_THROUGHPUT_REDUCTION,
    NIRISS_CLEARP_PSF_THROUGHPUT_REDUCTION,
    NIRCAM_WLP12_PSF_THROUGHPUT_REDUCTION,
)
...
def check_normalization(
    lib, lower_limit=0.80, upper_limit=1.0, renorm_psfs_above_1=True
):
...
def confirm_gridded_properties(
    filename,
    instrument,
    detector,
    filtername,
    ...
    if inst.upper() == "NIRISS" and filt in NIRISS_PUPIL_WHEEL_FILTERS:
        ...
        if pupil == "CLEARP":
            ...
            raise ValueError(
                (
                    "Pupil value is something other than "
    ...
    match = (
        inst.lower() == instrument.lower()
        and det.lower() == detector.lower()
        and filt.lower() == filtername.lower()
        and pupil.lower() == pupilname.lower()
    )
    if match and not default_psf:
        return full_filename
    elif (
        match
        and wfe_type == wavefront_error_type.lower()
        and realization == wavefront_error_group
    ):
        return full_filename
    else:
        return None
...
def fix_weak_lens_filter_pupil_values(filter_value, pupil_value):
    ...
    if save_pupil == "WLP4":
        ...
        raise ValueError(
            (
                "The pupil paired with WLP4, {}, does not "
...
def get_gridded_psf_library(
    ...
    if instrument.lower() == "fgs":
        ...
        if instrument.lower() == "niriss":
            ...
            filename_filter = filtername
            filename_pupil = (
                pupilname if "GDHS" not in pupilname else "clear"
            )  # for WFSC team practice purposes we don't produce DHS "PSFs"

        default_file_pattern = (
            "{}_{}_{}_{}_fovp*_samp*_npsf*_{}_realization{}.fits".format(
                instrument.lower(),
                detector.lower(),
    ...
    logger.info(
        "PSFs will be generated using: {}".format(os.path.abspath(library_file))
    )
    ...
    correct_norm, reason = check_normalization(
        library, lower_limit=check_min, upper_limit=check_max, renorm_psfs_above_1=True
    )
    if correct_norm:
        return library
    else:
...
def get_library_file(
    ...
    for filename in psf_files:
        try:
            ...
            # to match Mirage's expectations
            if (
                file_inst.upper() == "NIRISS"
                and file_filt in NIRISS_PUPIL_WHEEL_FILTERS
            ):
            ...
            # Same for NIRCam
            if (
                file_inst.upper() == "NIRCAM"
                and file_filt in NIRCAM_PUPIL_WHEEL_FILTERS
            ):
                save_filt = copy(file_filt)
                if file_pupil == "CLEAR":
                    if save_filt[0:2] == "F4":
            ...
            elif (file_inst.upper() == "NIRCAM") and (
                file_pupil.upper() in ["WLP4", "WLM4", "WLP12"]
            ):
                # If there is a weak lens listed in the library file's pupil value,
                # disentangle it into the correct filter and pupil values
                file_filt, file_pupil = fix_weak_lens_filter_pupil_values(
                    file_filt, file_pupil
                )
    ...
    if len(matches) == 1:
        ...
        logger.info(
            ...
            "wfe_group {}\nlibrary_path {}\n".format(
                instrument, detector, filt, pupil, wfe, wfe_group, library_path
            )
        )
        raise ValueError("No PSF library file found matching requested parameters.")
    elif len(matches) > 1:
        raise ValueError(
            "More than one PSF library file matches requested parameters: {}".format(
                matches
            )
        )
...
def get_psf_wings(
    ...
    # need to open and examine every file in the gridded library, which
    # saves at least a handful of seconds.
    default_file_pattern = "{}_{}_{}_{}_fovp*_samp*_{}_realization{}.fits".format(
        instrument.lower(),
        detector.lower(),
        filtername.lower(),
        pupilname.lower(),
    ...
    wings_file = None
    if len(default_matches) == 1:
        wings_file = confirm_gridded_properties(
            default_matches[0],
            instrument,
            detector,
            filtername,
    ...
    if wings_file is None:
        logger.info(
            (
                "No matching gridded PSF wing library file found based on filename pattern. Checking "
                "library files' metadata. This will be slower."
            )
        )
        # Find the file containing the PSF wings
        wings_file = get_library_file(
            instrument,
            detector,
            filtername,
            pupilname,
    ...
    for shape in psf_wing.shape:
        if shape % 2 == 0:
            logger.error(
                (
                    "WARNING: PSF wing file contains an even number of rows or columns. "
                    "These must be even."
                )
            )
            raise ValueError
...
def _load_itm_library(library_file):
    ...
    if data.shape == (2048, 2048):
        ...
        raise ValueError(
            "Expecting ITM data of size (2048, 2048), not {}".format(data.shape)
        )
